<div id="page-content" class="p20 clearfix">
    ...
    <div class="invoice-preview">
        <?php if ($this->login_user->user_type === "client" && $invoice_total_summary->balance_due >= 1 && count($payment_methods) && !$client_info->disable_online_payment) { ?>
            <div class="panel panel-default  p15 no-border clearfix">
                ...
                foreach ($payment_methods as $payment_method) {
                    ...
                    if ($invoice_total_summary->balance_due >= get_array_value($payment_method, "minimum_payment_amount")) {
                        if ($method_type == "stripe") {
                            $this->load->view("pinvoices/_stripe_payment_form", $pass_variables);
                        } else if ($method_type == "paypal_payments_standard") {
                            $this->load->view("pinvoices/_paypal_payments_standard_form", $pass_variables);
                        }
                    }
                }
                ?>
                <div class="pull-right">
                    <?php
                    echo "<div class='text-center'>" . anchor("pinvoices/download_pdf/" . $invoice_info->id, lang("download_pdf"), array("class" => "btn btn-default round")) . "</div>"
                    ?>
                </div>
        ...
        if ($show_close_preview) {
            echo "<div class='text-center'>" . anchor("pinvoices/view/" . $invoice_info->id, lang("close_preview"), array("class" => "btn btn-default round")) . "</div>";
        }
        ?>
