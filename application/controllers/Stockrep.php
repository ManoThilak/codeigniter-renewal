...
class Stockrep extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("stock/stockrep/index", $view_data);
    }
    ...
    function yearly() {
        $this->load->view("stock/stockrep/yearly_expenses");
    }

    //load custom expenses list
    function custom() {
        $this->load->view("stock/stockrep/custom_expenses");
    }
    ...
    function modal_form() {
       ...
       // $invoice_id = $this->input->post('invoice_id');
        ...
        $this->load->view('stock/stockrep/modal_form', $view_data);
    }
    ...
    function list_data() {
       ...
       // $sub_religion_id = $this->input->post('sub_religion_id');
        ...
        $this->Stockin_model->get_stockqty($start_date, $end_date);

        $this->Stockin_model->get_courierqty($start_date, $end_date);


        $list_data = $this->Stockin_model->searchqty($start_date, $end_date)->result();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_row($data, $custom_fields);
        }
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            $this->load->view("stock/stockrep/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function yearly_chart() {
        $this->load->view("stock/stockrep/yearly_chart");
    }
