...
class Bgiftpack extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("bgiftpack/index", $view_data);
    }
    ...
    function modal_form() {
      //  $this->access_only_allowed_members();
        ...
        $view_data['model_info'] = $this->Bgiftpack_model->get_one($client_id);
        ...
        $this->load->view('bgiftpack/modal_form', $view_data);
    }
    ...
    function customers_tele_fields() {
       // $this->access_only_allowed_members();
        ...
        //$view_data['package_dropdown'] = array("" => "-") + $this->Package_groups_model->get_dropdown_list(array("package_name","tot_contacts"), "id", array("deleted" => 0));
         ...
         $projects = $this->Bgiftpack_model->get_dropdown_list(array("id"), "id", array("client_id" => $project_client_id));
        ...
        $this->load->view('bgiftpack/customers_tele_modal_form', $view_data);
    }
    ...
    function save() {
        ...
        $save_id = $this->Bgiftpack_model->save($data, $client_id);
        if ($save_id) {
    ...
    function savetele() {
      ...
      //  $this->access_only_allowed_members_or_client_contact($client_id);
       ...
       // $this->Customers_model->insert_datas($telemarketer_id, $client_id, $from_id, $to_id);
         ...
         if ($this->Bgiftpack_model->insert_datas($client_id, $from_id, $to_id)) {
    ...
    function get_fromid_suggestion($client_id = 0, $telemarketer_id = 0) {
        ...
        $projects = $this->Bgiftpack_model->get_dropdown_list(array("id"), "id", array("client_id" => $client_id,"complete_id" => 1,"telemarketer_id" => $telemarketer_id,"status_id" => 1));
        $suggestion = array(array("id" => "", "text" => "-"));
        foreach ($projects as $key => $value) {
            $suggestion[] = array("id" => $key, "text" => $value);
        }
    ...
    function get_from_suggestion($client_id = 0) {
       // $this->access_only_allowed_members();
        ...
        $projects = $this->Bgiftpack_model->get_dropdown_list(array("id"), "id", array("client_id" => $client_id,"transfer_id" => 1, "bid" => 1,"bcouriersent_id" => 0,"giftforb_id" => 1));
    ...
    function get_totid_suggestion($client_id = 0, $fromid = 0, $toid = 0) {
        // $this->access_only_allowed_members();

        // $projects = $this->Customers_model->get_dropdown_list(array("id"), "id", array("client_id" => $client_id,"status_id" => 2));
        // $suggestion = array(array("id" => "", "text" => "-"));
        // foreach ($projects as $key => $value) {
        //     $suggestion[] = array("id" => $key, "text" => $value);
        // }
        ...
        //$telemarketer_id = $this->input->get('telemarketer_id');
        $from_id = $this->input->get('from_id');
        $to_id = $this->input->get('to_id');
         // console.log($from_id);
         //      console.log($to_id);
        $data     = $this->Bgiftpack_model->getAmountBySection($client_id,$from_id,$to_id);
    ...
    function delete() {
        ...
        if ($this->Bgiftpack_model->delete_client_and_sub_items($id)) {
    ...
    function list_data() {
       ...
       // $this->access_only_allowed_members();
        ...
        $list_data = $this->Bgiftpack_model->get_details($options)->result();
        $result = array();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Bgiftpack_model->get_details($options)->row();
    ...
    function view($client_id = 0, $tab = "") {
        ...
        if ($client_id) {
            ...
            $client_info = $this->Bgiftpack_model->get_details($options)->row();
            if ($client_info && !$client_info->is_lead) {
                ...
                $this->template->rander("bgiftpack/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function add_remove_star($client_id, $type = "add") {
        if ($client_id) {
            ...
            if ($type === "add") {
                $this->Bgiftpack_model->add_remove_star($client_id, $this->login_user->id, $type = "add");
                $this->load->view('bgiftpack/star/starred', $view_data);
            } else {
                $this->Bgiftpack_model->add_remove_star($client_id, $this->login_user->id, $type = "remove");
                $this->load->view('bgiftpack/star/not_starred', $view_data);
            }
        }
    }

    function show_my_starred_clients() {
        $view_data["clients"] = $this->Bgiftpack_model->get_starred_clients($this->login_user->id)->result();
        $this->load->view('bgiftpack/star/clients_list', $view_data);
    }
    ...
    function projects($client_id) {
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view("bgiftpack/projects/index", $view_data);
    }
    ...
    function payments($client_id) {
        ...
        if ($client_id) {
            $view_data["client_info"] = $this->Bgiftpack_model->get_one($client_id);
            $view_data['client_id'] = $client_id;
            $this->load->view("bgiftpack/payments/index", $view_data);
        }
    }
    ...
    function tickets($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("bgiftpack/tickets/index", $view_data);
        }
    }
    ...
    function invoices($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("bgiftpack/invoices/index", $view_data);
        }
    }
    ...
    function estimates($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("bgiftpack/estimates/estimates", $view_data);
        }
    }
    ...
    function estimate_requests($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("bgiftpack/estimates/estimate_requests", $view_data);
        }
    }
    ...
    function notes($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("bgiftpack/notes/index", $view_data);
        }
    }
    ...
    function files($client_id, $view_type = "") {
        ...
        if ($view_type == "page_view") {
            ...
            $this->template->rander("bgiftpack/files/index", $view_data);
        } else {
            $this->load->view("bgiftpack/files/index", $view_data);
        }
    }
    ...
    function file_modal_form() {
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view('bgiftpack/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='pull-left'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("bgiftpack/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("bgiftpack/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));

        if ($this->login_user->user_type == "staff") {
            $options .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("bgiftpack/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            $this->load->view("bgiftpack/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        $view_data['client_info'] = $this->Bgiftpack_model->get_one($view_data['user_info']->client_id);
        $view_data['tab'] = $tab;
        if ($view_data['user_info']->user_type === "client") {
            ...
            $this->template->rander("bgiftpack/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function my_preferences() {
        ...
        $this->load->view("bgiftpack/contacts/my_preferences", $view_data);
    }
    ...
    function contacts($client_id = 0) {
        ...
        $this->load->view("bgiftpack/contacts/index", $view_data);
    }
    ...
    function add_new_contact_modal_form() {
        ...
        $this->load->view('bgiftpack/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $this->load->view('bgiftpack/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function company_info_tab($client_id = 0) {
        if ($client_id) {
            ...
            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $this->load->view('bgiftpack/contacts/company_info_tab', $view_data);
        }
    }
    ...
    function save_contact() {
        ...
        $primary_contact = $this->Bgiftpack_model->get_primary_contact($client_id);
        if (!$primary_contact) {
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false) {
        ...
        $contact_link = anchor(get_uri("bgiftpack/contact_profile/" . $data->id), $full_name . $primary_contact) . $removal_request_pending;
        if ($this->login_user->user_type === "client") {
            $contact_link = $full_name; //don't show clickable link to client
        }

        $client_info = $this->Bgiftpack_model->get_one($data->client_id);

        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("bgiftpack/view/" . $data->client_id), $client_info->customer_name),
            $data->job_title,
            $data->email,
        ...
        $row_data[] = js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("bgiftpack/delete_contact"), "data-action" => "delete"));
    ...
    function invitation_modal() {
        ...
        $view_data["client_info"] = $this->Bgiftpack_model->get_one($client_id);
        $this->load->view('bgiftpack/contacts/invitation_modal', $view_data);
    }
    ...
    function users() {
        if ($this->login_user->user_type === "client") {
            ...
            $this->template->rander("bgiftpack/contacts/users", $view_data);
        }
    }
    ...
    function import_clients_modal_form() {
        $this->access_only_allowed_members();
        $this->load->view("bgiftpack/import_customers_modal_form");
    }
            ...
            foreach ($excel_file as $key => $value) {
                ...
                if ($key !== 0) {
                    ...
                    if ($client_data && count($client_data)) {
                        //save client data
                        $client_save_id = $this->Bgiftpack_model->save($client_data);
    ...
    function gdpr() {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        $this->load->view("bgiftpack/contacts/gdpr", $view_data);
    }
    ...
    private function _make_export_data($user_info) {
        ...
        //add company info
        $client_info = $this->Bgiftpack_model->get_one($user_info->client_id);
    ...
    function request_my_account_removal() {
        if (get_setting("enable_gdpr") && get_setting("clients_can_request_account_removal")) {
            ...
            redirect("bgiftpack/contact_profile/$user_id/gdpr");
        }
    }
