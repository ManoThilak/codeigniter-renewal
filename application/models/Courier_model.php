...
class Courier_model extends Crud_model {

    private $table = null;

    function __construct() {
        $this->table = 'courierdatas';
    ...
    function get_details($options = array()) {
       ...
       // // echo $this->db->last_query();
        ...
        // $this->db->query($sql2);
         ...
         $clients_table = $this->db->dbprefix('courierdatas');
        ...
        if ($start_date && $end_date) {
            ...
            $where .= " AND  DATE_FORMAT(birth_date, '%c-%d') 
                        ...
                        // $where .= " AND $clients_table.bcouriersent_id = '0'";
                         $where .= " AND $clients_table.couriersent_id = '0'";
        } else {
            //$where .= " AND FIND_IN_SET('0', $clients_table.id)";
            //$where .= " AND $clients_table.id='0'";
    ...
    function get_details_rep($options = array()) {
        ...
        if ($start_date && $end_date) {
            //$where .= " AND ($clients_table.birth_date BETWEEN '$start_date' AND '$end_date') ";
            $where .= " AND ($clients_table.assign_date BETWEEN '$start_date' AND '$end_date') ";
            // $where .= " AND ($clients_table.birth_date BETWEEN '$start_date' AND '$end_date' or 
            //   $clients_table.anniversary_date BETWEEN '$start_date' AND '$end_date')";
            // $where .= " AND  DATE_FORMAT(birth_date, '%c-%d') 
            //             BETWEEN DATE_FORMAT('$start_date', '%c-%d') 
            ...
            //             OR MONTH(birth_date) <= MONTH('$end_date')))";
                        // $where .= " AND $clients_table.transfer_id = '1'";
                        // $where .= " AND $clients_table.bcouriersent_id = '0'";
                         //$where .= " AND $clients_table.couriersent_id = '1'";
        } else {
            //$where .= " AND FIND_IN_SET('0', $clients_table.id)";
            //$where .= " AND $clients_table.id='0'";
    ...
    function insert_datas($client_id, $from_id, $to_id) {
        ...
        $sql = "INSERT INTO couriersentdatas (courier_id,telemarketer_id,customer_id,client_id,customer_name,birth_date,address,city,landmark,state,zip,phone,email,religion_id,sub_religion_id,gift_id,gift_for)
                SELECT id,telemarketer_id,customer_id,client_id,customer_name,birth_date,address,city,landmark,state,zip,phone,email,religion_id,sub_religion_id,gift_id,gift_for FROM courierdatas
         ...
         $this->db->query($sql);
           ...
           $sqlc = "UPDATE couriersentdatas 
            INNER JOIN clients ON couriersentdatas.client_id = clients.id
            SET couriersentdatas.salesmanager_id = clients.salesmanager_id";
            $this->db->query($sqlc);
    ...
    function insert_datascourier($from_id, $to_id) {
        //$clients_table = $this->db->dbprefix('customers');

        $sql = "INSERT INTO couriersentdatas (courier_id,telemarketer_id,customer_id,client_id,customer_name,birth_date,address,city,landmark,state,zip,phone,email,religion_id,sub_religion_id,gift_id,gift_for)
                SELECT id,telemarketer_id,customer_id,client_id,customer_name,birth_date,address,city,landmark,state,zip,phone,email,religion_id,sub_religion_id,gift_id,gift_for FROM courierdatas
                WHERE courierdatas.id >= $from_id AND courierdatas.id <= $to_id  and courierdatas.couriersent_id = 0 ";
        ...
        $sqll = "UPDATE  courierdatas set couriersent_id = 1 
         WHERE courierdatas.id >= $from_id AND courierdatas.id <= $to_id  and 
         courierdatas.couriersent_id = 0 ";
