...
class Pinvoices extends MY_Controller {
    ...
    function index() {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            $this->template->rander("pinvoices/index", $view_data);
        } else {
            $view_data["client_info"] = $this->Salesmanager_model->get_one($this->login_user->client_id);
            $view_data['client_id'] = $this->login_user->client_id;
            $view_data['page_type'] = "full";
            $this->template->rander("salesmanager/invoices/index", $view_data);
        }
    }

    //load the yearly view of invoice list 
    function yearly() {
        $this->load->view("pinvoices/yearly_invoices");
    }

    //load the recurring view of invoice list 
    function recurring() {
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown();
        $this->load->view("pinvoices/recurring_invoices_list", $view_data);
    }

    //load the custom view of invoice list 
    function custom() {
        $this->load->view("pinvoices/custom_invoices_list");
    }
    ...
    function modal_form() {
        ...
        $model_info = $this->Pinvoices_model->get_one($this->input->post('id'));
        ...
        $view_data['clients_dropdown'] = array("" => "-") + $this->Salesmanager_model->get_dropdown_list(array("company_name"), "id", array("is_lead" => 0));
        ...
        $labels = explode(",", $this->Pinvoices_model->get_label_suggestions());
        ...
        $this->load->view('pinvoices/modal_form', $view_data);
    ...
    function save() {
        ...
        // $files_data = move_files_from_temp_dir_to_permanent_dir($target_path, "pinvoices");
        ...
        //  if ($id) {
        //     $invoice_info = $this->Pinvoices_model->get_one($id);
        ...
        $invoice_id = $this->Pinvoices_model->save($invoice_data, $id);
        if ($invoice_id) {
            ...
            if ($is_clone && $main_invoice_id) {
                ...
                $invoice_items = $this->Pinvoice_items_model->get_all_where(array("invoice_id" => $main_invoice_id, "deleted" => 0))->result();
                ...
                foreach ($invoice_items as $invoice_item) {
                    ...
                    $invoice_item = $this->Pinvoice_items_model->save($invoice_item_data);
                }
            } else {
            ...
            if ($copy_items_from_estimate) {
                ...
                foreach ($estimate_items as $data) {
                    ...
                    $this->Pinvoice_items_model->save($invoice_item_data);
                }
            }
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Pinvoices_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        } else {
            if ($this->Pinvoices_model->delete($id)) {
    ...
    function list_data() {
        ...
        $list_data = $this->Pinvoices_model->get_details($options)->result();
        $result = array();
    ...
    function sub_invoices($recurring_invoice_id) {
        ...
        $this->load->view("pinvoices/sub_invoices", $view_data);
    }
    ...
    private function _row_data($id) {
        ...
        $data = $this->Pinvoices_model->get_details($options)->row();
    ...
    private function _make_row($data, $custom_fields) {
        $invoice_url = "";
        if ($this->login_user->user_type == "staff") {
            $invoice_url = anchor(get_uri("pinvoices/view/" . $data->id), get_invoice_id($data->id));
        } else {
            $invoice_url = anchor(get_uri("pinvoices/preview/" . $data->id), get_invoice_id($data->id));
        }
        ...
        //             $files_link .= js_anchor(" ", array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "class" => "pull-left font-22 mr10 $link", "title" => remove_file_prefix($file_name), "data-url" => get_uri("pinvoices/file_preview/" . $data->id . "/" . $key)));
        //         }
        //     }
        // }
        ...
        $row_data = array($invoice_url,
            anchor(get_uri("salesmanager/view/" . $data->client_id), $data->company_name),
    ...
    private function _make_options_dropdown($invoice_id = 0) {
        ...
        $edit = '<li role="presentation">' . modal_anchor(get_uri("pinvoices/modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit'), array("title" => lang('edit_invoice'), "data-post-id" => $invoice_id)) . '</li>';

        $delete = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), array('title' => lang('delete_invoice'), "class" => "delete", "data-id" => $invoice_id, "data-action-url" => get_uri("pinvoices/delete"), "data-action" => "delete")) . '</li>';
                                                                        //pexpenses/payment_modal_form
        $add_payment = '<li role="presentation">' . modal_anchor(get_uri("pinvoice_payments/payment_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_payment'), array("title" => lang('add_payment'), "data-post-invoice_id" => $invoice_id)) . '</li>';
    ...
    function view($invoice_id = 0) {
        ...
        if ($invoice_id) {
            ...
            $view_data = get_invoice_making_data_p($invoice_id);

            if ($view_data) {
                $view_data['invoice_status'] = $this->_get_invoice_status_label($view_data["invoice_info"], false);

                $this->template->rander("pinvoices/view", $view_data);
            } else {
                show_404();
            }
        }
    ...
    function item_modal_form() {
        ...
        $view_data['model_info'] = $this->Pinvoice_items_model->get_one($this->input->post('id'));
        if (!$invoice_id) {
            $invoice_id = $view_data['model_info']->invoice_id;
        }
        $view_data['invoice_id'] = $invoice_id;
        $this->load->view('pinvoices/item_modal_form', $view_data);
    }
    ...
    function save_item() {
        ...
        $invoice_item_id = $this->Pinvoice_items_model->save($invoice_item_data, $id);
        if ($invoice_item_id) {
            ...
            $item_info = $this->Pinvoice_items_model->get_details($options)->row();
    ...
    function delete_item() {
        ...
        $id = $this->input->post('id');
        if ($this->input->post('undo')) {
            if ($this->Pinvoice_items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Pinvoice_items_model->get_details($options)->row();
            ...
            if ($this->Pinvoice_items_model->delete($id)) {
                $item_info = $this->Pinvoice_items_model->get_one($id);
    ...
    function item_list_data($invoice_id = 0) {
        ...
        $list_data = $this->Pinvoice_items_model->get_details(array("invoice_id" => $invoice_id))->result();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("pinvoices/item_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_invoice'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("pinvoices/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    function update_item_sort_values($id = 0) {
        ...
        if ($sort_values) {
            ...
            foreach ($sort_array as $value) {
                ...
                $this->Pinvoice_items_model->save($data, $id);
            }
        }
    }
    ...
    function get_invoice_item_suggestion() {
        ...
        $items = $this->Pinvoice_items_model->get_item_suggestion($key);
    ...
    function get_invoice_item_info_suggestion() {
        $item = $this->Pinvoice_items_model->get_item_info_suggestion($this->input->post("item_name"));
        if ($item) {
    ...
    function preview($invoice_id = 0, $show_close_preview = false) {
        if ($invoice_id) {
            ...
            $this->template->rander("pinvoices/invoice_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function print_invoice($invoice_id = 0) {
        if ($invoice_id) {
            ...
            echo json_encode(array("success" => true, "print_view" => $this->load->view("pinvoices/print_invoice", $view_data, true)));
        } else {
            echo json_encode(array("success" => false, lang('error_occurred')));
        }
    }
    ...
    function download_pdf($invoice_id = 0) {
        ...
        if ($invoice_id) {
            $invoice_data = get_invoice_making_data_p($invoice_id);
    ...
    function send_invoice_modal_form($invoice_id) {
        ...
        if ($invoice_id) {
            ...
            $invoice_info = $this->Pinvoices_model->get_details($options)->row();
            ...
            $invoice_total_summary = $this->Pinvoices_model->get_invoice_total_summary($invoice_id);
            ...
            $parser_data["INVOICE_URL"] = get_uri("pinvoices/preview/" . $invoice_info->id);
            ...
            $this->load->view('pinvoices/send_invoice_modal_form', $view_data);
        } else {
            show_404();
        }
    }
    ...
    function send_invoice() {
        ...
        if (send_app_mail($contact->email, $subject, $message, array("attachments" => array(array("file_path" => $attachement_url)), "cc" => $cc, "bcc" => $bcc_emails))) {
            ...
            if ($this->Pinvoices_model->save($status_data, $invoice_id)) {
    ...
    function get_invoice_status_bar($invoice_id = 0) {
        ...
        $view_data["invoice_info"] = $this->Pinvoices_model->get_details(array("id" => $invoice_id))->row();
        $view_data['invoice_status_label'] = $this->_get_invoice_status_label($view_data["invoice_info"]);
        $this->load->view('pinvoices/invoice_status_bar', $view_data);
    }
    ...
    function discount_modal_form() {
        ...
        $view_data['model_info'] = $this->Pinvoices_model->get_one($invoice_id);

        $this->load->view('pinvoices/discount_modal_form', $view_data);
    }
    ...
    function save_discount() {
        ...
        $save_data = $this->Pinvoices_model->save($data, $invoice_id);
        if ($save_data) {
    ...
    function load_statistics_of_selected_currency($currency = "") {
     ...
     function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            $this->load->view("pinvoices/file_preview", $view_data);
        } else {
            show_404();
        }
    }
