        ...
        <?php } else { ?>
            <div class="tab-title clearfix">
                ...
                <div class="title-button-group">
                    <?php echo modal_anchor(get_uri("pinvoices/modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_invoice'), array("class" => "btn btn-default mb0", "data-post-client_id" => $client_id, "title" => lang('add_invoice'))); ?>
                </div>
            </div>
        <?php } ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        var currencySymbol = "<?php echo $supplier_info->currency_symbol; ?>";
        $("#invoice-table").appTable({
            source: '<?php echo_uri("pinvoices/invoice_list_data_of_client/" . $client_id) ?>',
            order: [[0, "desc"]],
            filterDropdown: [{name: "status", class: "w150", options: <?php $this->load->view("pinvoices/invoice_statuses_dropdown"); ?>}],
            columns: [
                ...
                {title: "<?php echo lang("payment_sent") ?>", "class": "w10p text-right"},
            ...
            summation: [{column: 6, dataType: 'currency', currencySymbol: currencySymbol}, {column: 7, dataType: 'currency', currencySymbol: currencySymbol}]
        });
    });
</script>