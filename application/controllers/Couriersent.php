...
class Couriersent extends MY_Controller {
    ...
    function index() {
       // $this->access_only_allowed_members();
        ...
        $this->template->rander("couriersent/index", $view_data);
    }
     ...
     function cindex() {
      //  $this->access_only_allowed_members();
        ...
        $view_data['telemarketers_dropdown'] = json_encode($this->get_telemarketers_dropdown(true));
        //$view_data['telemarketers_dropdown'] = array("" => "-") + $this->Telemarketers_model->get_dropdown_list(array("company_name"), "id", array("is_lead" => 0));
        $this->template->rander("couriersent/courierreport/index", $view_data);
            } else {
                //$view_data['groups_dropdown'] = json_encode($this->_get_groups_dropdown_select2_data(true));
        $view_data['clients_dropdown'] = json_encode($this->get_clients_dropdown(true));
        $view_data['salesmanager_dropdown'] = json_encode($this->get_salesmanager_dropdown(true));
        ...
        $view_data['telemarketers_dropdown'] = json_encode($this->get_telemarketers_dropdown(true));
        //$view_data['telemarketers_dropdown'] = array("" => "-") + $this->Telemarketers_model->get_dropdown_list(array("company_name"), "id", array("is_lead" => 0));
        $this->template->rander("clients/courierrep/index", $view_data);
            }
    }
    ...
    function modal_form() {
       // $this->access_only_allowed_members();
        ...
        $view_data['model_info'] = $this->Couriersent_model->get_one($client_id);
        ...
        $this->load->view('couriersent/modal_form', $view_data);
    }
    ...
    function customers_tele_fields() {
       // $this->access_only_allowed_members();
        ...
        //$view_data['package_dropdown'] = array("" => "-") + $this->Package_groups_model->get_dropdown_list(array("package_name","tot_contacts"), "id", array("deleted" => 0));
         ...
         $projects = $this->Couriersent_model->get_dropdown_list(array("id"), "id", array("client_id" => $project_client_id));
        ...
        $this->load->view('couriersent/customers_tele_modal_form', $view_data);
    }
    ...
    function save() {
       ...
       // $this->access_only_allowed_members_or_client_contact($client_id);
        ...
        $save_id = $this->Couriersent_model->save($data, $client_id);
        if ($save_id) {
    ...
    function savetele() {
       ...
       // $this->Customers_model->insert_datas($telemarketer_id, $client_id, $from_id, $to_id);
         ...
         if ($this->Couriersent_model->insert_datas($client_id, $from_id, $to_id)) {
    ...
    function get_fromid_suggestion($client_id = 0, $telemarketer_id = 0) {
      //  $this->access_only_allowed_members();
        ...
        $projects = $this->Couriersent_model->get_dropdown_list(array("id"), "id", array("client_id" => $client_id,"complete_id" => 1,"telemarketer_id" => $telemarketer_id,"status_id" => 1));
        $suggestion = array(array("id" => "", "text" => "-"));
        foreach ($projects as $key => $value) {
            $suggestion[] = array("id" => $key, "text" => $value);
        }
    ...
    function get_from_suggestion($client_id = 0) {
       // $this->access_only_allowed_members();
        ...
        $projects = $this->Couriersent_model->get_dropdown_list(array("id"), "id", array("client_id" => $client_id,"couriersent_id" => 0));
    ...
    function delete() {
       // $this->access_only_allowed_members();
        ...
        if ($this->Couriersent_model->delete_client_and_sub_items($id)) {
    ...
    function list_data() {
       ...
       // $this->access_only_allowed_members();
        ...
        $list_data = $this->Couriersent_model->get_details($options)->result();
        $result = array();
    ...
    function list_data_report() {
       ...
       // $this->access_only_allowed_members();
        ...
        $list_data = $this->Couriersent_model->get_details_rep($options)->result();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_rows($data, $custom_fields);
        }
        echo json_encode(array("data" => $result));
    }
    function list_data_client_report() {
    ...
    private function _row_data($id) {
        ...
        $data = $this->Couriersent_model->get_details($options)->row();
        return $this->_make_row($data, $custom_fields);
    }

    /* prepare a row of client list table */

    private function _make_rows($data, $custom_fields) {
        ...
        $row_data = array($data->id,
            ...
            format_to_date($data->courier_date, false),
        ...
        // $row_data[] = modal_anchor(get_uri("bgiftpack/modal_form"), "<i class='fa fa-note'></i>", array("class" => "edit", "title" => lang('edit_customer'), "data-post-id" => $data->id));
        //         . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_telemarketer'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("giftpack/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($client_id = 0, $tab = "") {
        ...
        if ($client_id) {
            ...
            $client_info = $this->Couriersent_model->get_details($options)->row();
            if ($client_info && !$client_info->is_lead) {
                ...
                $this->template->rander("couriersent/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function add_remove_star($client_id, $type = "add") {
        if ($client_id) {
            ...
            if ($type === "add") {
                $this->Couriersent_model->add_remove_star($client_id, $this->login_user->id, $type = "add");
                $this->load->view('couriersent/star/starred', $view_data);
            } else {
                $this->Couriersent_model->add_remove_star($client_id, $this->login_user->id, $type = "remove");
                $this->load->view('couriersent/star/not_starred', $view_data);
            }
        }
    }

    function show_my_starred_clients() {
        $view_data["clients"] = $this->Couriersent_model->get_starred_clients($this->login_user->id)->result();
        $this->load->view('couriersent/star/clients_list', $view_data);
    }
    ...
    function projects($client_id) {
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view("couriersent/projects/index", $view_data);
    }
    ...
    function payments($client_id) {
        ...
        if ($client_id) {
            $view_data["client_info"] = $this->Couriersent_model->get_one($client_id);
            $view_data['client_id'] = $client_id;
            $this->load->view("couriersent/payments/index", $view_data);
        }
    }
    ...
    function tickets($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("couriersent/tickets/index", $view_data);
        }
    }
    ...
    function invoices($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("couriersent/invoices/index", $view_data);
        }
    }
    ...
    function estimates($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("couriersent/estimates/estimates", $view_data);
        }
    }
    ...
    function estimate_requests($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("couriersent/estimates/estimate_requests", $view_data);
        }
    }
    ...
    function notes($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("couriersent/notes/index", $view_data);
        }
    }
    ...
    function files($client_id, $view_type = "") {
        ...
        if ($view_type == "page_view") {
            ...
            $this->template->rander("couriersent/files/index", $view_data);
        } else {
            $this->load->view("couriersent/files/index", $view_data);
        }
    }
    ...
    function file_modal_form() {
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view('couriersent/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='pull-left'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("couriersent/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("couriersent/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));

        if ($this->login_user->user_type == "staff") {
            $options .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("couriersent/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            $this->load->view("couriersent/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        $view_data['client_info'] = $this->Couriersent_model->get_one($view_data['user_info']->client_id);
        $view_data['tab'] = $tab;
        if ($view_data['user_info']->user_type === "client") {
            ...
            $this->template->rander("couriersent/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function my_preferences() {
        ...
        $this->load->view("couriersent/contacts/my_preferences", $view_data);
    }
    ...
    function contacts($client_id = 0) {
        ...
        $this->load->view("couriersent/contacts/index", $view_data);
    }
    ...
    function add_new_contact_modal_form() {
        ...
        $this->load->view('couriersent/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $this->load->view('couriersent/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function company_info_tab($client_id = 0) {
        if ($client_id) {
           // $this->access_only_allowed_members_or_client_contact($client_id);
            ...
            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $this->load->view('couriersent/contacts/company_info_tab', $view_data);
        }
    }
    ...
    function save_contact() {
      ...
      //  $this->access_only_allowed_members_or_contact_personally($contact_id);
        ...
        $primary_contact = $this->Couriersent_model->get_primary_contact($client_id);
        if (!$primary_contact) {
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false) {
        ...
        $contact_link = anchor(get_uri("couriersent/contact_profile/" . $data->id), $full_name . $primary_contact) . $removal_request_pending;
        if ($this->login_user->user_type === "client") {
            $contact_link = $full_name; //don't show clickable link to client
        }

        $client_info = $this->Couriersent_model->get_one($data->client_id);

        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("couriersent/view/" . $data->client_id), $client_info->customer_name),
            $data->job_title,
            $data->email,
        ...
        $row_data[] = js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("couriersent/delete_contact"), "data-action" => "delete"));
    ...
    function invitation_modal() {
       ...
       // $this->access_only_allowed_members_or_client_contact($client_id);
        ...
        $view_data["client_info"] = $this->Couriersent_model->get_one($client_id);
        $this->load->view('couriersent/contacts/invitation_modal', $view_data);
    }
    ...
    function users() {
        if ($this->login_user->user_type === "client") {
            ...
            $this->template->rander("couriersent/contacts/users", $view_data);
        }
    }
    ...
    function import_clients_modal_form() {
       // $this->access_only_allowed_members();
        $this->load->view("couriersent/import_customers_modal_form");
    }
            ...
            foreach ($excel_file as $key => $value) {
                ...
                if ($key !== 0) {
                    ...
                    foreach ($value as $row_data_key => $row_data_value) {
                        ...
                        if ($row_data_value) {
                            // if  ($header_key_value == "id") {
                            //     $client_data["id"] = $row_data_value;
                            // } else if ($header_key_value == "courier_name") {
                            //     $client_data["courier_name"] = $row_data_value;
                            // } else if ($header_key_value == "tracking_number") {
                            //     $client_data["tracking_number"] = $row_data_value;
                            // } else {
                        ...
                        $this->Couriersent_model->updatetrackingnumber($cid, $cname, $cnum);
                    ...
                    if ($client_data && count($client_data)) {
                        //save client data
                        $client_save_id = $this->Couriersent_model->save($client_data);
    ...
    private function _row_data_validation_and_get_error_message($key, $data, $has_customer_name) {
        ...
        if ($header_value == "tracking_number" && !$data) {
            return lang("import_client_error_trackingnum_field_required");
        }
    ...
    function gdpr() {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        $this->load->view("couriersent/contacts/gdpr", $view_data);
    }
    ...
    private function _make_export_data($user_info) {
        ...
        //add company info
        $client_info = $this->Couriersent_model->get_one($user_info->client_id);
    ...
    function request_my_account_removal() {
        if (get_setting("enable_gdpr") && get_setting("clients_can_request_account_removal")) {
            ...
            redirect("couriersent/contact_profile/$user_id/gdpr");
        }
    }
    ...
    public function generatemultiple()
    {
        ...
        $id_cards = $this->load->view('couriersent/generatemultiple', $data, true);
        echo $id_cards;
    }
