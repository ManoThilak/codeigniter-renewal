...
class Suppliers extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->init_permission_checker("supplier");
    }
    ...
    function index() {
        ...
        $view_data['category_dropdown'] = json_encode($this-> _get_category_dropdown_select2_data_supplier(true));
        $view_data['area_dropdown'] = json_encode($this-> _get_area_dropdown_select2_data_supplier(true));
        $view_data['city_dropdown'] = json_encode($this-> _get_city_dropdown_select2_data_supplier(true));
        $view_data['status_dropdown'] = json_encode($this-> _get_status_dropdown_select2_data_supplier(true));
    ...
    function location() {
        ...
        $this->template->rander("suppliers/location", $view_data);
    }

     private function _get_category_dropdown() {
        $godowns = $this->Category_model->get_all_where(array("deleted" => 0), 0, 0, "title")->result();

        $godowns_dropdown = array(array("id" => "", "text" => "- " . lang("category") . " -"));
    ...
    function modal_form() {
        ...
        $model_info = $this->Suppliers_model->get_one($client_id);
        $view_data['category_dropdown'] = array("0" => "-") + $this->Category_model->get_dropdown_list(array("title"));
        $view_data['city_dropdown'] = array("0" => "-") + $this->City_model->get_dropdown_list(array("title"));
        $view_data['area_dropdown'] = array("0" => "-") + $this->Area_model->get_dropdown_list(array("title"));
        $view_data["currency_dropdown"] = $this->_get_currency_dropdown_select2_data();
        $view_data['status_dropdown'] = array("enabled" => "Enabled") + array("disabled" => "Disabled");

        //prepare groups dropdown list
        // $view_data['groups_dropdown'] = $this->_get_groups_dropdown_select2_data();
        $view_data['groups_dropdown'] = $this->_get_groups_dropdown_select2_data_supplier();


        $projects = $this->Area_model->get_dropdown_list(array("title"), "id", array("city_id" => $model_info ->area_id));
        ...
        //get custom fields
        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("suppliers", $client_id, $this->login_user->is_admin, $this->login_user->user_type)->result();

        $this->load->view('suppliers/modal_form', $view_data);
    }

    function modal_forms() {
        $this->access_only_allowed_members();
        ...
        $this->load->view('suppliers/modal_forms', $view_data);
    }
    ...
    function save() {
        ...
        if (get_setting("disallow_duplicate_client_company_name") == "1" && $this->Suppliers_model->is_duplicate_company_name($data["company_name"], $client_id)) {
            echo json_encode(array("success" => false, 'message' => lang("account_already_exists_for_your_company_name")));
            exit();
        }


        $save_id = $this->Suppliers_model->save($data, $client_id);
        if ($save_id) {
            save_custom_fields("suppliers", $save_id, $this->login_user->is_admin, $this->login_user->user_type);
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $group_list = "";
        if ($data->supplier_groups) {
            $groups = explode(",", $data->supplier_groups);
...
$group_list1 = "<ul style='width: 150px; height: 150px; padding-left: 0px;'><iframe src='https://www.google.com/maps?q=$latitude,$longitude&hl=es;z=14&output=embed' style='width: 115%; height: 100%;'> </iframe> </ul>";
        ...
        $row_data = array($data->id,
            anchor(get_uri("suppliers/view/" . $data->id), $data->company_name),
        ...
        $row_data[] = modal_anchor(get_uri("suppliers/modal_forms"), "<i class='fa fa-eye'></i>", array("class" => "edit", "title" => lang('view_location'), "data-post-id" => $data->id)) . modal_anchor(get_uri("suppliers/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_location'), "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_location'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("suppliers/delete"), "data-action" => "delete-confirmation") ) ;
    ...
    function view($client_id = 0, $tab = "") {
        ...
        if ($client_id) {
            ...
            $supplier_info = $this->Suppliers_model->get_details($options)->row();
            if ($supplier_info && !$supplier_info->is_lead) {
        ...
        $view_data['area_dropdown'] = array("0" => "-") + $this->Area_model->get_dropdown_list(array("title"));
                ...
                $this->template->rander("suppliers/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function add_remove_star($client_id, $type = "add") {
        if ($client_id) {
            ...
            if ($type === "add") {
                $this->Suppliers_model->add_remove_star($client_id, $this->login_user->id, $type = "add");
                $this->load->view('suppliers/star/starred', $view_data);
            } else {
                $this->Suppliers_model->add_remove_star($client_id, $this->login_user->id, $type = "remove");
                $this->load->view('suppliers/star/not_starred', $view_data);
            }
        }
    }

    function show_my_starred_suppliers() {
        $view_data["suppliers"] = $this->Suppliers_model->get_starred_suppliers($this->login_user->id)->result();
        $this->load->view('suppliers/star/suppliers_list', $view_data);
    }
    ...
    function projects($client_id) {
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view("suppliers/projects/index", $view_data);
    }
    ...
    function payments($client_id) {
        ...
        if ($client_id) {
            $view_data["supplier_info"] = $this->Suppliers_model->get_one($client_id);
            $view_data['client_id'] = $client_id;
            $this->load->view("suppliers/payments/index", $view_data);
        }
    }
    ...
    function tickets($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("suppliers/tickets/index", $view_data);
        }
    }
    ...
    function invoices($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("suppliers/invoices/index", $view_data);
        }
    }
    ...
    function estimates($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("suppliers/estimates/estimates", $view_data);
        }
    }
    ...
    function estimate_requests($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("suppliers/estimates/estimate_requests", $view_data);
        }
    }
    ...
    function notes($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("suppliers/notes/index", $view_data);
        }
    }
    ...
    function events($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $view_data['calendar_filter_dropdown'] = $this->get_calendar_filter_dropdown("supplier");
    ...
    function files($client_id) {
        ...
        $view_data['files'] = $this->General_files_p_model->get_details($options)->result();
        $view_data['client_id'] = $client_id;
        $this->load->view("suppliers/files/index", $view_data);
    }

    /* file upload modal */

    function file_modal_form() {
        $view_data['model_info'] = $this->General_files_p_model->get_one($this->input->post('id'));
        $client_id = $this->input->post('client_id') ? $this->input->post('client_id') : $view_data['model_info']->client_id;

        $this->access_only_allowed_members();

        $view_data['client_id'] = $client_id;
        $this->load->view('suppliers/files/modal_form', $view_data);
    }
        ...
        if ($files && get_array_value($files, 0)) {
            foreach ($files as $file) {
                ...
                if ($file_info) {
                    ...
                    $success = $this->General_files_p_model->save($data);
                } else {
                    $success = false;
                }
            }
    ...
    function files_list_data($client_id = 0) {
        ...
        $options = array("client_id" => $client_id);
        $list_data = $this->General_files_p_model->get_details($options)->result();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='pull-left'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("suppliers/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("suppliers/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));

        $options .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("suppliers/delete_file"), "data-action" => "delete-confirmation"));
    ...
    function view_file($file_id = 0) {
        $file_info = $this->General_files_p_model->get_details(array("id" => $file_id))->row();
        ...
        if ($file_info) {
            ...
            $this->load->view("suppliers/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function download_file($id) {
        ...
        $file_info = $this->General_files_p_model->get_one($id);
    ...
    function delete_file() {
        ...
        $id = $this->input->post('id');
        $info = $this->General_files_p_model->get_one($id);

        if (!$info->client_id) {
            redirect("forbidden");
        }

        if ($this->General_files_p_model->delete($id)) {
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        $view_data['supplier_info'] = $this->Suppliers_model->get_one($view_data['user_info']->client_id);
        $view_data['tab'] = $tab;
        if ($view_data['user_info']->user_type === "client") {
            ...
            $this->template->rander("suppliers/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function my_preferences() {
        ...
        $this->load->view("suppliers/contacts/my_preferences", $view_data);
    }
    ...
    function contacts($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("suppliers/contacts/index", $view_data);
        }
    }
    ...
    function add_new_contact_modal_form() {
        ...
        $this->load->view('suppliers/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $this->load->view('suppliers/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function company_info_tab($client_id = 0) {
        ...
        $view_data['status_dropdown'] = array("enabled" => "Enabled") + array("disabled" => "Disabled");
            ...
            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $this->load->view('suppliers/contacts/company_info_tab', $view_data);
        }
    }



    function map($client_id = 0) {
        if ($client_id) {
            $this->access_only_allowed_members_or_client_contact($client_id);
        ...
        $view_data['area_dropdown'] = array("0" => "-") + $this->Area_model->get_dropdown_list(array("title"));
            ...
            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $this->load->view('suppliers/contacts/map', $view_data);
        }
    }
    ...
    function save_contact() {
        ...
        $primary_contact = $this->Suppliers_model->get_primary_contact($client_id);
        if (!$primary_contact) {
    ...
    private function _make_contact_row($data, $custom_fields) {
        ...
        $contact_link = anchor(get_uri("suppliers/contact_profile/" . $data->id), $full_name . $primary_contact);
        ...
        $row_data[] = js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("suppliers/delete_contact"), "data-action" => "delete"));
    ...
    function invitation_modal() {
        ...
        $view_data["supplier_info"] = $this->Suppliers_model->get_one($client_id);
        $this->load->view('suppliers/contacts/invitation_modal', $view_data);
    }
    ...
    function users() {
        if ($this->login_user->user_type === "client") {
            ...
            $this->template->rander("suppliers/contacts/users", $view_data);
        }
    }
    ...
    function import_suppliers_modal_form() {
        $this->access_only_allowed_members();
        $this->load->view("suppliers/import_suppliers_modal_form");
    }

    function save_supplier_from_excel_file() {
        if ($this->validate_import_suppliers_file_data(true)) {
            ...
            foreach ($excel_file as $key => $value) {
                ...
                if ($key !== 0) {
                    ...
                    foreach ($value as $row_data_key => $row_data_value) {
                        ...
                        if ($row_data_value) {
                            if ($header_key_value == "supplier_groups") {
                                //we've to make client groups data differently
                                $supplier_data["group_ids"] = $this->_get_client_group_ids($row_data_value);
                            } else if ($header_key_value == "contact_first_name") {
                                $supplier_contact_data["first_name"] = $row_data_value;
                            } else if ($header_key_value == "contact_last_name") {
                                $supplier_contact_data["last_name"] = $row_data_value;
                            } else if ($header_key_value == "contact_email") {
                                $supplier_contact_data["email"] = $row_data_value;
                            } else {
                                $supplier_data[$header_key_value] = $row_data_value;
                            }
                        }

                        //add created time
                        $now = get_current_utc_time();
                        $supplier_data["created_date"] = $now;
                        $supplier_contact_data["created_at"] = $now;
                    }

                    if ($supplier_data && count($supplier_data)) {
                        //save supplier data
                        $supplier_save_id = $this->Suppliers_model->save($supplier_data);

                        //add supplier id to contact data
                        if ($supplier_save_id) {
                            $supplier_contact_data["client_id"] = $supplier_save_id;
                            $this->Users_model->save($supplier_contact_data);
                        }
                    }
                }
            }
    ...
    private function _get_client_group_ids($client_groups_data) {
        $explode_supplier_groups = explode(", ", $supplier_groups_data);
        if ($explode_supplier_groups && count($explode_supplier_groups)) {
            $groups_ids = "";

            foreach ($explode_supplier_groups as $group) {
                $group_id = "";
                $existing_group = $this->Supplier_groups_model->get_one_where(array("title" => $group, "deleted" => 0));
                if ($existing_group->id) {
                    //supplier group exists, add the group id
                    $group_id = $existing_group->id;
                } else {
                    //supplier group doesn't exists, create a new one and add group id
                    $group_data = array("title" => $group);
                    $group_id = $this->Supplier_groups_model->save($group_data);
                }
    ...
    function validate_import_suppliers_file() {
        $this->access_only_allowed_members();
    ...
    function validate_import_suppliers_file_data($check_on_submit = false) {
        $this->access_only_allowed_members();
