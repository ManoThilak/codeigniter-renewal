...
class Invoices_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($start_date && $end_date) {
            $where .= " AND ($invoices_table.next_followup_date BETWEEN '$start_date' AND '$end_date') ";
        }
        ...
        LEFT JOIN salesmanager ON salesmanager.id= $invoices_table.entry_id
        ...
        LEFT JOIN (SELECT invoice_id, SUM(total) AS invoice_value FROM $invoice_items_table WHERE deleted=0 GROUP BY invoice_id) AS items_table ON items_table.invoice_id = $invoices_table.id 
         ...
         -- LEFT JOIN (SELECT invoice_id, GROUP_CONCAT(invoice_items.title ORDER BY invoice_items.title ASC SEPARATOR ', ') AS combined_titles WHERE deleted=0 GROUP BY invoice_id) AS sevice_table ON invoice_items.invoice_id = $invoices_table.id 
         LEFT JOIN (SELECT invoice_id,sum(total) as total_amt, GROUP_CONCAT(invoice_items.title ORDER BY invoice_items.title ASC SEPARATOR ', ') AS combined_titles FROM invoice_items WHERE deleted = 0 GROUP BY invoice_id) AS sevice_table ON sevice_table.invoice_id = invoices.id
    ...
    function get_details_expiry($options = array()) {
        $invoices_table = $this->db->dbprefix('invoices');
        $clients_table = $this->db->dbprefix('clients');
        $projects_table = $this->db->dbprefix('projects');
        $taxes_table = $this->db->dbprefix('taxes');
        ...
        $join_custom_fieds
        WHERE $invoices_table.deleted = 0  $where order by $invoices_table.due_date asc";
        return $this->db->query($sql);
        // $this->db->query($sql);
        // echo $this->db->last_query();
    } 

    function get_details_portfolio($options = array()) {
        $invoices_table = $this->db->dbprefix('invoices');
        $clients_table = $this->db->dbprefix('clients');
        $projects_table = $this->db->dbprefix('projects');
        $taxes_table = $this->db->dbprefix('taxes');
        ...
        $state_id = get_array_value($options, "state_id");
        if ($state_id) {
            $where .= " AND $clients_table.state_id=$state_id";
        }

        $city_id = get_array_value($options, "city_id");
        if ($city_id) {
            $where .= " AND $clients_table.city_id=$city_id";
        }
        ...
        $bcategory_id = get_array_value($options, "bcategory_id");
        if ($bcategory_id) {
            $where .= " AND FIND_IN_SET($bcategory_id, $invoices_table.labels) > 0";
        }
        ...
        $sql = "SELECT $invoices_table.*,category_subquery.bcategory,salesmanager.company_name as entry_idd, $clients_table.currency, $clients_table.salesmanager_id, $clients_table.currency_symbol, $clients_table.company_name, state.title as state, city.title as city, $projects_table.title AS project_title,sevice_table.combined_titles,sevice_table.total_amt,
           $invoice_value_calculation_query AS invoice_value, IFNULL(payments_table.payment_received,0) AS payment_received, tax_table.percentage AS tax_percentage, tax_table2.percentage AS tax_percentage2, tax_table3.percentage AS tax_percentage3, CONCAT($users_table.first_name, ' ',$users_table.last_name) AS cancelled_by_user $select_custom_fieds
        FROM $invoices_table
        LEFT JOIN $clients_table ON $clients_table.id= $invoices_table.client_id
        LEFT JOIN state ON $clients_table.state_id= state.id
        LEFT JOIN city ON $clients_table.city_id= city.id
        LEFT JOIN $projects_table ON $projects_table.id= $invoices_table.project_id
        LEFT JOIN $users_table ON $users_table.id= $invoices_table.cancelled_by
        LEFT JOIN salesmanager ON salesmanager.id= $invoices_table.entry_id
        ...
        LEFT JOIN (SELECT invoice_id, SUM(total) AS invoice_value FROM $invoice_items_table WHERE deleted=0 GROUP BY invoice_id) AS items_table ON items_table.invoice_id = $invoices_table.id 
         ...
         -- LEFT JOIN (SELECT invoice_id, GROUP_CONCAT(invoice_items.title ORDER BY invoice_items.title ASC SEPARATOR ', ') AS combined_titles WHERE deleted=0 GROUP BY invoice_id) AS sevice_table ON invoice_items.invoice_id = $invoices_table.id 
         LEFT JOIN (SELECT invoice_id,sum(total) as total_amt, GROUP_CONCAT(invoice_items.title ORDER BY invoice_items.title ASC SEPARATOR ', ') AS combined_titles FROM invoice_items WHERE deleted = 0 GROUP BY invoice_id) AS sevice_table ON sevice_table.invoice_id = invoices.id
         LEFT JOIN (SELECT invoices.id, GROUP_CONCAT(bcategory.title) AS bcategory FROM invoices
            LEFT JOIN bcategory ON FIND_IN_SET(bcategory.id, invoices.labels) > 0 GROUP BY invoices.id) 
            AS category_subquery ON invoices.id = category_subquery.id
        $join_custom_fieds
        WHERE $invoices_table.deleted = 0  $where order by $invoices_table.due_date asc";
        return $this->db->query($sql);
        // $this->db->query($sql);
