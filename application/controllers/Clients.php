...
class Clients extends MY_Controller {
    ...
    function modal_form() {
        ...
        $model_info = $this->Clients_model->get_one($this->input->post('id'));



        if ($city_id) {
            $state_id = $this->City_model->get_one($city_id)->state_id;
            $model_info->state_id = $state_id;
        }


        $project_state_id = $state_id;
        if ($model_info->state_id) {
            $project_state_id = $model_info->state_id;
        }
        ...
        $view_data['model_info'] = $this->Clients_model->get_one($client_id);
         $projects = $this->City_model->get_dropdown_list(array("title"), "id", array("state_id" => $project_state_id));
        ...
        $view_data['state_id'] = $state_id;
        $view_data['city_id'] = $city_id;




        $view_data['state_dropdown'] = array("" => "-") + $this->State_model->get_dropdown_list(array("title"), "id", array("deleted" => 0));
        //$view_data['model_infos'] = $this->Users_model->get_one($client_id);
        ...
        if ($this->login_user->salesmanager_id === "0") {
            $view_data['salesmanager_dropdown'] = array("" => "-") + $this->Salesmanager_model->get_dropdown_list(array("company_name"), "id", array("is_lead" => 0));
            } else {
            $view_data['salesmanager_dropdown'] =  $this->Salesmanager_model->get_dropdown_list(array("company_name"), "id", array("is_lead" => 0,"id" => $this->login_user->salesmanager_id));
            }
    ...
    function save() {
       ...
       //$this->Clients_model->emptyfiles($client_id);
        ...
        validate_submitted_data(array(
            ...
            // "sphone" => "required",
            //"sphone" => "numeric",
            //"sphone" => "regex_match[/^[0-9]{10}$/]",
            // "sphone" => "required|min_length[10]",
        ...
        $data = array(
            ...
            "scompany_name" => $this->input->post('scompany_name'),
        ...
        //             $img_name = $client_id . '.' . $fileInfo['extension'];
        //             move_uploaded_file($_FILES["file"]["tmp_name"], "./uploads/student_images/" . $img_name);
        //             $data_img = array('id' => $client_id, 'image' => 'uploads/student_images/' . $img_name);
        ...
        if ($client_id) {
            $expense_info = $this->Clients_model->get_one($client_id);
...
// //    }
    ...
    function get_city_suggestion($client_id = 0) {
      //  $this->access_only_allowed_members();

        $projects = $this->City_model->get_dropdown_list(array("title"), "id", array("state_id" => $client_id));
        $suggestion = array(array("id" => "", "text" => "-"));
        foreach ($projects as $key => $value) {
            $suggestion[] = array("id" => $key, "text" => $value);
        }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($this->login_user->salesmanager_id === "0") {
            $com = anchor(get_uri("clients/view/" . $data->id), $data->company_name);
            } else {
            $com = $data->company_name;
            }
    ...
    function files($client_id, $view_type = "") {
        ...
        if ($view_type == "page_view") {
            ...
            $this->template->rander("clients/files/index", $view_data);
        } else {
    ...
    function company_info_tab($client_id = 0) {
        ...
        $model_info = $this->Clients_model->get_one($client_id);
    ...
    function gdpr() {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        $this->load->view("clients/contacts/gdpr", $view_data);
    }
    ...
    function request_my_account_removal() {
        if (get_setting("enable_gdpr") && get_setting("clients_can_request_account_removal")) {
            ...
            redirect("clients/contact_profile/$user_id/gdpr");
        }
    }
