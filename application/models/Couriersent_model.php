...
class Couriersent_model extends Crud_model {

    private $table = null;

    function __construct() {
        $this->table = 'couriersentdatas';
    ...
    function get_details($options = array()) {
       ...
       // // echo $this->db->last_query();
        ...
        if ($start_date && $end_date) {
            ...
            $where .= " AND  DATE_FORMAT(birth_date, '%c-%d') 
                        ...
                        // $where .= " AND $clients_table.bcouriersent_id = '0'";
                         $where .= " AND $clients_table.tracking_num is NULL";
        } else {
            //$where .= " AND FIND_IN_SET('0', $clients_table.id)";
            //$where .= " AND $clients_table.id='0'";
            ...
            // $where .= " AND ($clients_table.birth_date BETWEEN curdate() and date_add(curdate(), INTERVAL 30 day) or 
            //   $clients_table.anniversary_date BETWEEN curdate() and date_add(curdate(), INTERVAL 30 day))";
            // -------- $where .= " AND (DATE_ADD(birth_date,INTERVAL YEAR(DATE_ADD(CURDATE(),INTERVAL 10 DAY))-YEAR(birth_date) + IF(DAYOFYEAR(DATE_ADD(CURDATE(),INTERVAL 10 DAY)) > DAYOFYEAR(birth_date),1,0) YEAR) BETWEEN DATE_ADD(CURDATE(),INTERVAL 10 DAY) AND DATE_ADD(DATE_ADD(CURDATE(),INTERVAL 10 DAY), INTERVAL 10 DAY))"; ------------------------------
            // $where .= " AND $clients_table.transfer_id = '1'";
            // $where .= " AND $clients_table.bcouriersent_id = '0'";
            $where .= " AND $clients_table.tracking_num is NULL";
        }
        ...
        $sql = "SELECT $clients_table.*, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS primary_contact, $users_table.id AS primary_contact_id, $users_table.image AS contact_avatar,  project_table.total_projects, $payment_value_select AS payment_received $select_custom_fieds,
                ...
                $clientname_table.scompany_name as scompany_name,$clientname_table.sphone as ssphone,complete_master.title as status, telemarketers.company_name as telemarketer_name,religion.title as religion,sub_religion.title as sub_religion,gift_master.title as gift,customers.id  as custid,customers.customer_name as customer_name,customers.address as caddress,customers.city as ccity,customers.state as cstate,customers.zip as czip,customers.landmark as clandmark,customers.phone as cphone
        FROM $clients_table 
        ...
        LEFT JOIN customers ON customers.id = $clients_table.customer_id
        ...
        $join_custom_fieds               
        WHERE $clients_table.deleted=0 $where 
         ORDER BY $clients_table.id ASC,$clients_table.client_id ASC";
        return $this->db->query($sql);
       // echo $this->db->last_query();
    ...
    function get_details_rep($options = array()) {
        ...
        $end_date = get_array_value($options, "end_date");
        if ($start_date && $end_date) {
            $where .= " AND ($clients_table.courier_date BETWEEN '$start_date' AND '$end_date') ";
            //$where .= " AND ($clients_table.birth_date BETWEEN '$start_date' AND '$end_date') ";
            //$where .= " AND ($clients_table.anniversary_date BETWEEN '$start_date' AND '$end_date') ";
            // $where .= " AND ($clients_table.birth_date BETWEEN '$start_date' AND '$end_date' or 
            //   $clients_table.anniversary_date BETWEEN '$start_date' AND '$end_date')";
        ...
        $client_id = get_array_value($options, "client_id");
        if ($client_id) {
            $where .= " AND $clients_table.client_id='$client_id'";
        } 
        $salesmanager_id = get_array_value($options, "salesmanager_id");
        if ($salesmanager_id) {
            $where .= " AND $clients_table.salesmanager_id='$salesmanager_id'";
        } 
        ...
        $sql = "SELECT $clients_table.*, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS primary_contact, $users_table.id AS primary_contact_id, $users_table.image AS contact_avatar,  project_table.total_projects, $payment_value_select AS payment_received $select_custom_fieds,
                ...
                package_groups.package_name as package_groups,package_types.title as package_types,
                $clientname_table.scompany_name as scompany_name,$clientname_table.sphone as ssphone,complete_master.title as status, telemarketers.company_name as telemarketer_name,religion.title as religion,sub_religion.title as sub_religion,gift_master.title as gift,customers.id  as custid,customers.customer_name as customer_name,customers.address as caddress,customers.city as ccity,customers.state as cstate,customers.zip as czip,customers.landmark as clandmark,customers.phone as cphone,
                salesmanager.company_name as salesmanname
        FROM $clients_table 
        ...
        $join_custom_fieds               
        WHERE $clients_table.deleted=0 $where 
         ORDER BY $clients_table.courier_date ASC";
        return $this->db->query($sql);
       // echo $this->db->last_query();
    ...
    function insert_datas($client_id, $from_id, $to_id) {
        ...
        $sql = "INSERT INTO couriersentdatas (courier_id,telemarketer_id,customer_id,client_id,customer_name,birth_date,address,city,landmark,state,zip,phone,email,religion_id,sub_religion_id,gift_id,gift_for)
                SELECT id,telemarketer_id,id,client_id,customer_name,birth_date,address,city,landmark,state,zip,phone,email,religion_id,sub_religion_id,gift_id,gift_for FROM courierdatas
                WHERE courierdatas.id >= $from_id AND courierdatas.id <= $to_id and courierdatas.client_id = $client_id and courierdatas.couriersent_id = 0 ";

         $this->db->query($sql);


        $sqll = "UPDATE  courierdatas set couriersent_id = 1 
         WHERE courierdatas.id >= $from_id AND courierdatas.id <= $to_id and courierdatas.client_id = $client_id and 
         courierdatas.couriersent_id = 0 ";
    ...
    function updatetrackingnumber($cid, $cname, $cnum) {
        ...
        //Update onetime or regular values
         $sql = "UPDATE customers
                    INNER JOIN clients ON customers.client_id = clients.id
                    SET customers.bcouriersent_id = 0
                    WHERE clients.package_type_id = 3 and customers.bid=0";
         $this->db->query($sql);

         $sqla = "UPDATE customers
                    INNER JOIN clients ON customers.client_id = clients.id
                    SET customers.acouriersent_id = 0
                    WHERE clients.package_type_id = 3 and customers.aid=0";
         $this->db->query($sqla);


        $sqll = "UPDATE  couriersentdatas set tracking_num = '$cnum', courier_name = '$cname'
         WHERE couriersentdatas.id = $cid ";
