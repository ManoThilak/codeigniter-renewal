...
class Giftpack_model extends Crud_model {
    ...
    function get_details($options = array()) {
       ...
       // // echo $this->db->last_query();
        ...
        $sql1="UPDATE customers set aid = 0";
         $this->db->query($sql1);
         $sql2="UPDATE customers set aid = 1 WHERE transfer_id = 1 and  DATE_ADD(anniversary_date,INTERVAL YEAR(DATE_ADD(CURDATE(),INTERVAL 10 DAY))-YEAR(anniversary_date) + IF(DAYOFYEAR(DATE_ADD(CURDATE(),INTERVAL 10 DAY)) > DAYOFYEAR(anniversary_date),1,0) YEAR) BETWEEN DATE_ADD(CURDATE(),INTERVAL 10 DAY) AND DATE_ADD(DATE_ADD(CURDATE(),INTERVAL 10 DAY), INTERVAL 30 DAY)";
         $this->db->query($sql2);



          $sql11="UPDATE customers set giftfora_id = 0";
         $this->db->query($sql11);
         $sql12="UPDATE customers set giftfora_id = 1 WHERE transfer_id = 1 and (giftfor_id = 3 or giftfor_id = 2)";
         $this->db->query($sql12);
        ...
        if ($start_date && $end_date) {
            ...
            //             OR
            //               DATE_FORMAT(anniversary_date, '%c-%d') 
            //             BETWEEN DATE_FORMAT('$start_date', '%c-%d') 
            //             AND DATE_FORMAT('$end_date', '%c-%d') OR (MONTH('$start_date') > MONTH('$end_date')
            //             AND (MONTH(anniversary_date) >= MONTH('$start_date') 
            //             OR MONTH(anniversary_date) <= MONTH('$end_date'))))";
            $where .= " AND  DATE_FORMAT(anniversary_date, '%c-%d') 
                        BETWEEN DATE_FORMAT('$start_date', '%c-%d') 
                        AND DATE_FORMAT('$end_date', '%c-%d') OR (MONTH('$start_date') > MONTH('$end_date')
                        AND (MONTH(anniversary_date) >= MONTH('$start_date') 
                        OR MONTH(anniversary_date) <= MONTH('$end_date')))";
                        $where .= "AND $clients_table.transfer_id = '1' ";
                         $where .= "AND $clients_table.acouriersent_id = '0'";
                         $where .= "AND ($clients_table.giftfora_id = '1')";
        } else {
            //$where .= " AND FIND_IN_SET('0', $clients_table.id)";
            //$where .= " AND $clients_table.id='0'";
            ...
            //   $clients_table.anniversary_date BETWEEN curdate() and date_add(curdate(), INTERVAL 30 day))";
            $where .= " AND 
                    DATE_ADD(anniversary_date,INTERVAL YEAR(DATE_ADD(CURDATE(),INTERVAL 10 DAY))-YEAR(anniversary_date) + IF(DAYOFYEAR(DATE_ADD(CURDATE(),INTERVAL 10 DAY)) > DAYOFYEAR(anniversary_date),1,0) YEAR) BETWEEN DATE_ADD(CURDATE(),INTERVAL 10 DAY) AND DATE_ADD(DATE_ADD(CURDATE(),INTERVAL 10 DAY), INTERVAL 30 DAY)";
                    $where .= "AND $clients_table.transfer_id = '1'";
                    $where .= "AND $clients_table.acouriersent_id = '0'";
                    $where .= "AND ($clients_table.giftfora_id = '1')";
        }
        ...
        WHERE $clients_table.deleted=0 $where 
         ...
         ORDER BY DAYOFYEAR($clients_table.anniversary_date) < DAYOFYEAR(CURDATE()) , DAYOFYEAR($clients_table.anniversary_date)";
         //ORDER BY $clients_table.client_id ASC";
        return $this->db->query($sql);
       // echo $this->db->last_query();
    ...
    public function getAmountBySection($client_id, $fromid, $toid) {
        ...
        $sql = "select count(*) as result from customers
            where customers.client_id = ". $client_id ." and customers.id >= ". $fromid ." and customers.id <= ". $toid ." and transfer_id = 1 and aid = 1 and acouriersent_id = 0 and giftfora_id = 1 ";
    ...
    function insert_datas($client_id, $from_id, $to_id) {
        ...
        $sql = "INSERT INTO courierdatas (telemarketer_id,customer_id,client_id,customer_name,            anniversary_date,address,city,landmark,state,zip,phone,email,religion_id,sub_religion_id,gift_id,gift_for)
                SELECT telemarketer_id,id,client_id,customer_name,anniversary_date,address,city,landmark,state,zip,phone,email,religion_id,sub_religion_id,gift_id,'Anniversary Gift' FROM customers
                WHERE customers.id >= $from_id AND customers.id <= $to_id and customers.client_id = $client_id and customers.aid = 1 and customers.transfer_id=1";

         $this->db->query($sql);

         $sqll = "UPDATE  customers set acouriersent_id = 1 
         WHERE customers.id >= $from_id AND customers.id <= $to_id and customers.client_id = $client_id and 
         customers.aid = 1 and customers.transfer_id=1";
