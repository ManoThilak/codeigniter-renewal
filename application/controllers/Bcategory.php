...
class Bcategory extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("bcategory/index");
    }
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Bcategory_model->get_one($this->input->post('id'));

        $this->load->view('bcategory/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        $item_id = $this->Bcategory_model->save($item_data, $id);
        if ($item_id) {
            $options = array("id" => $item_id);
            $item_info = $this->Bcategory_model->get_details($options)->row();
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Bcategory_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Bcategory_model->get_details($options)->row();
            ...
            if ($this->Bcategory_model->delete($id)) {
                $item_info = $this->Bcategory_model->get_one($id);
    ...
    function list_data() {
        ...
        $list_data = $this->Bcategory_model->get_details()->result();
        $result = array();
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            // $type,
            // $data->rate,
            modal_anchor(get_uri("bcategory/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_bcategory'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("bcategory/delete"), "data-action" => "delete"))
        );
    }
