...
if (!function_exists('get_invoice_making_data_p')) {

    function get_invoice_making_data_p($invoice_id) {
        $ci = get_instance();
        $invoice_info = $ci->Pinvoices_model->get_details(array("id" => $invoice_id))->row();
        if ($invoice_info) {
            $data['invoice_info'] = $invoice_info;
            $data['client_info'] = $ci->Salesmanager_model->get_one($data['invoice_info']->client_id);
            $data['invoice_items'] = $ci->Pinvoice_items_model->get_details(array("invoice_id" => $invoice_id))->result();
            $data['invoice_status_label'] = get_invoice_status_label($invoice_info);
            $data["invoice_total_summary"] = $ci->Pinvoices_model->get_invoice_total_summary($invoice_id);
