...
class Salesmanager extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("salesmanager/index", $view_data);
    }
    ...
    function modal_form() {
       ...
       // $view_data["ticket_id"] = $this->input->post('ticket_id'); //needed only when loading from the ticket's details view and created by unknown client
        $view_data['model_info'] = $this->Salesmanager_model->get_one($client_id);
        ...
        $this->load->view('salesmanager/modal_form', $view_data);
    }
    ...
    function save() {
      ...
      //  $this->access_only_allowed_members_or_client_contact($client_id);
        ...
        if (get_setting("disallow_duplicate_client_company_name") == "1" && $this->Salesmanager_model->is_duplicate_company_name($data["company_name"], $client_id)) {
        ...
        $save_id = $this->Salesmanager_model->save($data, $client_id);
        ...
        $user_data["role_id"] = 2;
        $user_data["salesmanager_id"] = $save_id;
    ...
    function delete() {
       // $this->access_only_allowed_members();
        ...
        if ($this->Salesmanager_model->delete_client_and_sub_items($id)) {
    ...
    function list_data() {
       ...
       // $this->access_only_allowed_members();
        ...
        $list_data = $this->Salesmanager_model->get_details($options)->result();
        $result = array();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Salesmanager_model->get_details($options)->row();
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $row_data = array($data->id,
            anchor(get_uri("salesmanager/view/" . $data->id), $data->company_name),
            $data->phone,
            $data->comm_amt,
        ...
        if ($this->login_user->is_admin) {
            $row_data[] = modal_anchor(get_uri("salesmanager/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_salesmanager'), "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_salesmanager'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("salesmanager/delete"), "data-action" => "delete-confirmation"));
                } else {
                    $row_data[] = modal_anchor(get_uri("salesmanager/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_salesmanager'), "data-post-id" => $data->id))
                ;
                }
    ...
    function view($client_id = 0, $tab = "") {
       // $this->access_only_allowed_members();
        ...
        if ($client_id) {
            ...
            $client_info = $this->Salesmanager_model->get_details($options)->row();
            if ($client_info && !$client_info->is_lead) {
                ...
                $this->template->rander("salesmanager/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function add_remove_star($client_id, $type = "add") {
        if ($client_id) {
            ...
            if ($type === "add") {
                $this->Salesmanager_model->add_remove_star($client_id, $this->login_user->id, $type = "add");
                $this->load->view('salesmanager/star/starred', $view_data);
            } else {
                $this->Salesmanager_model->add_remove_star($client_id, $this->login_user->id, $type = "remove");
                $this->load->view('salesmanager/star/not_starred', $view_data);
            }
        }
    }

    function show_my_starred_clients() {
        $view_data["clients"] = $this->Salesmanager_model->get_starred_clients($this->login_user->id)->result();
        $this->load->view('salesmanager/star/sales_list', $view_data);
    }
    ...
    function projects($client_id) {
        ...
        $this->load->view("salesmanager/projects/index", $view_data);
    }
    ...
    function payments($client_id) {
        ...
        if ($client_id) {
            $view_data["client_info"] = $this->Salesmanager_model->get_one($client_id);
            $view_data['client_id'] = $client_id;
            $this->load->view("salesmanager/payments/index", $view_data);
        }
    }
    ...
    function tickets($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("salesmanager/tickets/index", $view_data);
        }
    }
    ...
    function invoices($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("salesmanager/invoices/index", $view_data);
        }
    }
    ...
    function estimates($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("salesmanager/estimates/estimates", $view_data);
        }
    }
    ...
    function estimate_requests($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("salesmanager/estimates/estimate_requests", $view_data);
        }
    }
    ...
    function notes($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = $client_id;
            $this->load->view("salesmanager/notes/index", $view_data);
        }
    }
    ...
    function files($client_id, $view_type = "") {
        ...
        if ($view_type == "page_view") {
            ...
            $this->template->rander("salesmanager/files/index", $view_data);
        } else {
            $this->load->view("salesmanager/files/index", $view_data);
        }
    }
    ...
    function file_modal_form() {
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view('salesmanager/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='pull-left'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("salesmanager/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("salesmanager/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));

        if ($this->login_user->user_type == "staff") {
            $options .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("salesmanager/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            $this->load->view("salesmanager/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        $view_data['client_info'] = $this->Salesmanager_model->get_one($view_data['user_info']->client_id);
        $view_data['tab'] = $tab;
        if ($view_data['user_info']->user_type === "client") {
            ...
            $this->template->rander("salesmanager/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function my_preferences() {
        ...
        $this->load->view("salesmanager/contacts/my_preferences", $view_data);
    }
    ...
    function contacts($client_id = 0) {
        ...
        $this->load->view("salesmanager/contacts/index", $view_data);
    }
    ...
    function add_new_contact_modal_form() {
        ...
        $this->load->view('salesmanager/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $this->load->view('salesmanager/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function company_info_tab($client_id = 0) {
        if ($client_id) {
            ...
            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $this->load->view('salesmanager/contacts/company_info_tab', $view_data);
        }
    }
    ...
    function save_contact() {
        ...
        $primary_contact = $this->Salesmanager_model->get_primary_contact($client_id);
        if (!$primary_contact) {
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false) {
        ...
        $contact_link = anchor(get_uri("salesmanager/contact_profile/" . $data->id), $full_name . $primary_contact) . $removal_request_pending;
        if ($this->login_user->user_type === "client") {
            $contact_link = $full_name; //don't show clickable link to client
        }

        $client_info = $this->Salesmanager_model->get_one($data->client_id);

        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("salesmanager/view/" . $data->client_id), $client_info->company_name),
            $data->job_title,
            $data->email,
        ...
        $row_data[] = js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("salesmanager/delete_contact"), "data-action" => "delete"));
    ...
    function invitation_modal() {
        ...
        $view_data["client_info"] = $this->Salesmanager_model->get_one($client_id);
        $this->load->view('salesmanager/contacts/invitation_modal', $view_data);
    }
    ...
    function users() {
        if ($this->login_user->user_type === "client") {
            ...
            $this->template->rander("salesmanager/contacts/users", $view_data);
        }
    }
    ...
    function import_clients_modal_form() {
        $this->access_only_allowed_members();
        $this->load->view("salesmanager/import_sales_modal_form");
    }
            ...
            foreach ($excel_file as $key => $value) {
                ...
                if ($key !== 0) {
                    ...
                    if ($client_data && count($client_data)) {
                        //save client data
                        $client_save_id = $this->Salesmanager_model->save($client_data);
    ...
    function gdpr() {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        $this->load->view("salesmanager/contacts/gdpr", $view_data);
    }
    ...
    private function _make_export_data($user_info) {
        ...
        //add company info
        $client_info = $this->Salesmanager_model->get_one($user_info->client_id);
    ...
    function request_my_account_removal() {
        if (get_setting("enable_gdpr") && get_setting("clients_can_request_account_removal")) {
            ...
            redirect("salesmanager/contact_profile/$user_id/gdpr");
        }
    }
