...
class Clients_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($client_id) {
            $where .= " AND $clients_table.salesmanager_id='$client_id'";
        }
        ...
        if ($salesmanager_id) {
            $where .= " AND salesmanager.id='$salesmanager_id'";
        }
        ...
        $sql = "SELECT $clients_table.*, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS primary_contact, $users_table.id AS primary_contact_id, $users_table.image AS contact_avatar,  project_table.total_projects,gift_table.total_gifts, $payment_value_select AS payment_received $select_custom_fieds,
                ...
                package_groups.package_name as package_groups,package_types.title as package_types,state.title as state,city.title as city
        FROM $clients_table
        ...
        LEFT JOIN (SELECT client_id, COUNT(id) AS total_projects FROM customers WHERE deleted=0 GROUP BY client_id) AS project_table ON project_table.client_id= $clients_table.id


        LEFT JOIN (SELECT client_id, COUNT(id) AS total_gifts FROM couriersentdatas WHERE deleted=0 and MONTH(courier_date) = MONTH(CURRENT_DATE()) AND YEAR(courier_date) = YEAR(CURRENT_DATE()) GROUP BY client_id) AS gift_table ON gift_table.client_id= $clients_table.id
        ...
        LEFT JOIN (SELECT client_id, SUM(payments_table.payment_received) as payment_received, $invoice_value_calculation_query as invoice_value FROM $invoices_table
                   ...
                   LEFT JOIN (SELECT invoice_id, SUM(total) AS invoice_value FROM $invoice_items_table WHERE deleted=0 GROUP BY invoice_id) AS items_table ON items_table.invoice_id=$invoices_table.id AND $invoices_table.deleted=0 
        ...
        $join_custom_fieds               
        WHERE $clients_table.deleted=0 $where order by $clients_table.id desc";
        return $this->db->query($sql);
       // echo $this->db->last_query();


        //137th line--client dashboard affect invoice value// LEFT JOIN (SELECT invoice_id, SUM(total) AS invoice_value FROM $invoice_items_table WHERE deleted=0 GROUP BY invoice_id) AS items_table ON items_table.invoice_id=$invoices_table.id AND $invoices_table.deleted=0 AND $invoices_table.status='not_paid'  WHERE invoices_table.status='not_paid'  GROUP BY $invoices_table.client_id) AS invoice_details ON invoice_details.client_id= $clients_table.id 
    }
    ...
    function is_email_exists($email, $id = 0) {
        ...
        WHERE $users_table.deleted=0 AND $users_table.email='$email' ";
    ...
    function get_total_client($id ="") {
         ...
         function get_total_salesmanager($id ="") {
            //$clients_table = $this->db->dbprefix('clients');
            $sql = "SELECT  COUNT(salesmanager.id) as total
            FROM salesmanager                
        ...
        function get_total_telecallers($id ="") {
            //$clients_table = $this->db->dbprefix('clients');
            $sql = "SELECT  COUNT(telemarketers.id) as total
            FROM telemarketers                
            WHERE telemarketers.deleted=0 ";
