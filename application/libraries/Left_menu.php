...
class Left_menu {
    ...
    private function _get_sidebar_menu_items($type = "") {
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "client_default") {
            ...
            $access_customers = get_array_value($permissions, "customers");
            $access_salesmanager = get_array_value($permissions, "salesmanager");
            $access_telemarketing = get_array_value($permissions, "telemarketing");
            $access_telecallers = get_array_value($permissions, "telecallers");
            $access_giftpack = get_array_value($permissions, "giftpack");
            $access_stock = get_array_value($permissions, "stock");
            $access_courier = get_array_value($permissions, "courier");
            $access_couriersent = get_array_value($permissions, "couriersent");
            ...
            if ($this->ci->login_user->is_admin || $access_customers) {
                // $sidebar_menu["customers"] = array("name" => "customers", "url" => "customers", "class" => "fa-user-plus");
            }
            ...
            //     $sidebar_menu["clients"] = array("name" => "clients", "url" => "clients", "class" => "fa-briefcase",
            //         "submenu" => array(
            //             array("name" => "clients", "url" => "clients"),
            //             array("name" => "invoices", "url" => "invoices", "class" => "fa-file-text-o"),
            ...
            //     $sidebar_menu["salesmanager"] = array("name" => "salesmanager", "url" => "estimates", "class" => "fa-id-badge",
            //         "submenu" => array(
            //             array("name" => "salesmanager", "url" => "salesmanager"),
            //             //array("name" => "invoices", "url" => "pinvoices", "class" => "fa-file-text-o"),
            //             // array("name" => "estimate_forms", "url" => "estimate_requests/estimate_forms", "class" => "fa-file-o")
            //         )
            //     );
            // }
            ...
            if ($this->ci->login_user->is_admin ) {
                // $sidebar_menu["salesmanager"] = array("name" => "salesmanager", "url" => "salesmanager", "class" => "fa-handshake-o");
            }
            // if ($this->ci->login_user->is_admin || $access_salesmanager) {
            //     $sidebar_menu["salesmanagerc"] = array("name" => "salesmanagerc", "url" => "salesmancom", "class" => "fa-handshake-o");
            // }

            if ($this->ci->login_user->is_admin || $access_telemarketing) {
                // $sidebar_menu["telemarketers"] = array("name" => "telemarketers", "url" => "telemarketers", "class" => "fa-id-badge");
            }

            if ($this->ci->login_user->is_admin || $access_telecallers) {
                // $sidebar_menu["manage_telemarketers"] = array("name" => "manage_telemarketers", "url" => "mtelemarketers", "class" => "fa-vcard-o");
            }

            if ($this->ci->login_user->is_admin || $access_giftpack) {
                // $sidebar_menu["giftpack"] = array("name" => "giftpack", "url" => "giftpack", "class" => "fa-gift");
            }

            if ($this->ci->login_user->is_admin || $access_giftpack) {
                // $sidebar_menu["bgiftpack"] = array("name" => "bgiftpack", "url" => "bgiftpack", "class" => "fa-birthday-cake");
            }

             if ($this->ci->login_user->is_admin || $access_stock) {
                // $sidebar_menu["stock"] = array("name" => "stock", "url" => "stockin", "class" => "fa-database");
            }
            //  if ($this->ci->login_user->is_admin || $access_stock) {
            //     $sidebar_menu["stockrep"] = array("name" => "stockrep", "url" => "stockrep", "class" => "fa-database");
            // }
            if ($this->ci->login_user->is_admin || $access_courier) {
                // $sidebar_menu["courier"] = array("name" => "courier", "url" => "courier", "class" => "fa-send-o");
            }
             if ($this->ci->login_user->is_admin || $access_couriersent) {
                // $sidebar_menu["couriersent"] = array("name" => "couriersent", "url" => "couriersent", "class" => "fa-send-o");
            }
            ...
            if ((get_setting("module_invoice") == "1" || get_setting("module_expense") == "1") && ($this->ci->login_user->is_admin || $access_expense || $access_invoice)) {
                ...
                if (get_setting("module_invoice") == "1" && ($this->ci->login_user->is_admin || $access_invoice)) {
                    // $finance_submenu[] = array("name" => "income", "url" => "invoice_payments", "class" => "fa-money");
                ...
                if (get_setting("module_invoice") == "1" && ($this->ci->login_user->is_admin || $access_invoice)) {
                    // $finance_submenu[] = array("name" => "invoices", "url" => "invoices", "class" => "fa-money");
                    $finance_url = "invoices";
                ...
                // $sidebar_menu["finance"] = array("name" => "accounts", "url" => $finance_url, "class" => "fa-money", "submenu" => $finance_submenu);
            }
            ...
            if ((get_setting("module_invoice") == "1" || get_setting("module_expense") == "1") && ($this->ci->login_user->is_admin || $access_expense || $access_invoice || $access_couriersent || $access_courier || $access_telecallers || $access_salesmanager || $access_stock || $access_giftpack)) {
