...
class Pnotes extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("pnotes/index");
    }

    function modal_form() {
        $view_data['model_info'] = $this->Pnotes_model->get_one($this->input->post('id'));
        ...
        $labels = explode(",", $this->Pnotes_model->get_label_suggestions($this->login_user->id));
        ...
        $this->load->view('pnotes/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if ($id) {
            $note_info = $this->Pnotes_model->get_one($id);
        ...
        $save_id = $this->Pnotes_model->save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->Pnotes_model->delete($id)) {
            //delete the files
    ...
    function list_data($type = "", $id = 0) {
        ...
        $list_data = $this->Pnotes_model->get_details($options)->result();
        $result = array();
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Pnotes_model->get_details($options)->row();
    ...
    private function _make_row($data) {
        ...
        $title = modal_anchor(get_uri("pnotes/view/" . $data->id), $public_icon . $data->title, array("title" => lang('note'), "data-post-id" => $data->id));
        ...
        if ($data->files) {
            ...
            if (count($files)) {
                foreach ($files as $key => $value) {
                    ...
                    $files_link .= js_anchor(" ", array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "class" => "pull-left font-22 mr10 $link", "title" => remove_file_prefix($file_name), "data-url" => get_uri("pnotes/file_preview/" . $data->id . "/" . $key)));
                }
            }
        }

        //only creator and admin can edit/delete notes
        $actions = modal_anchor(get_uri("pnotes/view/" . $data->id), "<i class='fa fa-bolt'></i>", array("class" => "edit", "title" => lang('note_details'), "data-modal-title" => lang("note"), "data-post-id" => $data->id));
        if ($data->created_by == $this->login_user->id || $this->login_user->is_admin) {
            $actions = modal_anchor(get_uri("pnotes/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_note'), "data-post-id" => $data->id))
                    . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_note'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("pnotes/delete"), "data-action" => "delete-confirmation"));
        }
    ...
    function view() {
        ...
        $model_info = $this->Pnotes_model->get_details(array("id" => $this->input->post('id')))->row();

        $this->validate_access_to_note($model_info);

        $view_data['model_info'] = $model_info;
        $this->load->view('pnotes/view', $view_data);
    }
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            $this->load->view("pnotes/file_preview", $view_data);
        } else {
            show_404();
        }
    }
