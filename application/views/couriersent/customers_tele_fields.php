...
<div class="form-group">
    <label for="client" class="<?php echo $label_column; ?>"><?php echo lang('client_name'); ?><small class="req"> *</small></label>
    <div class="<?php echo $field_column; ?>">
        <?php
        echo form_dropdown("client", $clients_dropdown, array($model_info->client_id), "class='select2 validate-hidden' id='client' data-rule-required='true' data-msg-required='" . lang('field_required') . "'");
                ?>
    </div>
</div>
<!-- <div class="form-group">
...
$("#client_id").select2().on("change", function () {
    ...
    //console.log(client_id);
        if ($(this).val()) {
            ...
            appLoader.show({container: "#invoice-porject-dropdown-section"});
            $.ajax({
                url: "<?php echo get_uri("couriersent/get_from_suggestion") ?>" + "/" + client_id,
                dataType: "json",
                success: function (result) {
                    $("#from_id").show().val("");
...
//         });
        ...
        if ("<?php echo $project_id; ?>") {
         ...
         $("#client").select2().on("change", function () {
            ...
            // } else {
                $.ajax({
                    url:"<?php echo get_uri("couriersent/generatemultiple") ?>",
                    //url: '<?php echo site_url("couriersent/generatemultiple") ?>',
                    type: 'post',
                    dataType: "html",
