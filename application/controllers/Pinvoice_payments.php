...
class Pinvoice_payments extends MY_Controller {
    ...
    function index() {
        if ($this->login_user->user_type === "staff") {
            ...
            $this->template->rander("pinvoices/payment_received", $view_data);
        } else {
            $view_data["client_info"] = $this->Salesmanager_model->get_one($this->login_user->client_id);
            $view_data['client_id'] = $this->login_user->client_id;
            $view_data['page_type'] = "full";
            $this->template->rander("salesmanager/payments/index", $view_data);
        }
    }
    ...
    function yearly() {
        $this->load->view("pinvoices/yearly_payments");
    }

    //load custom payment list
    function custom() {
        $this->load->view("pinvoices/custom_payments_list");
    }
    ...
    function payment_modal_form() {
        ...
        $view_data['model_info'] = $this->Pinvoice_payments_model->get_one($this->input->post('id'));
        ...
        $this->load->view('pinvoices/payment_modal_form', $view_data);
    }
    ...
    function save_payment() {
        ...
        $invoice_payment_id = $this->Pinvoice_payments_model->save($invoice_payment_data, $id);
        if ($invoice_payment_id) {
            ...
            $item_info = $this->Pinvoice_payments_model->get_details($options)->row();
    ...
    function delete_payment() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Pinvoice_payments_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Pinvoice_payments_model->get_details($options)->row();
            ...
            if ($this->Pinvoice_payments_model->delete($id)) {
                $item_info = $this->Pinvoice_payments_model->get_one($id);
    ...
    private function _make_payment_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("pinvoice_payments/payment_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_payment'), "data-post-id" => $data->id, "data-post-invoice_id" => $data->invoice_id,))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("pinvoice_payments/delete_payment"), "data-action" => "delete"))
        );
    }
    ...
    function pay_invoice_via_stripe() {
        ...
        $redirect_to = "pinvoices/preview/$invoice_id";
        ...
        try {
            ...
            if ($charge->paid) {
                ...
                $invoice_payment_id = $this->Pinvoice_payments_model->save($invoice_payment_data);
                if ($invoice_payment_id) {
    ...
    function yearly_chart() {
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown();
        $this->load->view("pinvoices/yearly_payments_chart", $view_data);
    }
    ...
    function yearly_chart_data() {
        ...
        if ($year) {
            $currency = $this->input->post("currency");
            $payments = $this->Pinvoice_payments_model->get_yearly_payments_chart($year, $currency);
            $values = array();
