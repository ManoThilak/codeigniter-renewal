...
class Telemarketers_model extends Crud_model {

    private $table = null;

    function __construct() {
        $this->table = 'telemarketers';
    ...
    function get_details($options = array()) {
       ...
       //          $clientname_table.company_name as client_name,status_master.title as status
       //  FROM $clients_table 
    ...
    function save_email_from_users() {
        

        $sql = "UPDATE telemarketers t1 
        INNER JOIN users t2 
        ON t1.id = t2.telecallers_id
    ...
    function delete_client_and_sub_items($client_id) {
        ...
        //delete contacts
        $delete_contacts_sql = "UPDATE $users_table SET $users_table.deleted=1 WHERE $users_table.telecallers_id=$client_id; ";
    ...
    function insert_datas($telemarketer_id, $client_id, $from_id, $to_id) {
        ...
        $sql = "INSERT INTO teledatas (telemarketer_id,customer_id,client_id,customer_name,birth_date,            anniversary_date,address,city,state,zip,phone,email)
         ...
         $sqll = "UPDATE  customers set status_id = 1 
         WHERE customers.id >= $from_id AND customers.id <= $to_id and customers.client_id = $client_id";
