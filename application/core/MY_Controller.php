...
class MY_Controller extends CI_Controller {
    ...
    protected function _get_package_data() {
        //$currency = array(array("id" => "", "text" => "-"));
        foreach (get_package_dropdown() as $value) {
            $currency[] = array("id" => $value, "text" => $value);
        }
        return $currency;
    }
    ...
    protected function get_clients_dropdown($show_header = false) {
    //     $clients_dropdown = array("" => "-");
    //     $clients_json_dropdown = array(array("id" => "", "text" => "- " . lang("client") . " -"));
    //     $clients = $this->Clients_model->get_all_where(array("deleted" => 0), 0, 0, "is_lead")->result();
    ...
    // }
        ...
        $client_groups = $this->Clients_model->get_all()->result();
        $groups_dropdown = array();

        if ($show_header) {
            $groups_dropdown[] = array("id" => "", "text" => "- " . lang("client") . " -");
        }
    ...
    protected function get_salesmanager_dropdown($show_header = false) {
   
        $client_groups = $this->Salesmanager_model->get_all()->result();
        $groups_dropdown = array();

        if ($show_header) {
            $groups_dropdown[] = array("id" => "", "text" => "- " . lang("salesmanager") . " -");
        }
    ...
    protected function get_from_dropdown($show_header = false) {
    
        $client_groups = $this->Customers_model->get_all()->result();
        $groups_dropdown = array();

        if ($show_header) {
            $groups_dropdown[] = array("id" => "", "text" => "- " . lang("from_id") . " -");
        }

        foreach ($client_groups as $group) {
            $groups_dropdown[] = array("id" => $group->id, "text" => $group->id);
        }
        return $groups_dropdown;
    }
    protected function get_to_dropdown($show_header = false) {
    
        $client_groups = $this->Customers_model->get_all()->result();
        $groups_dropdown = array();

        if ($show_header) {
            $groups_dropdown[] = array("id" => "", "text" => "- " . lang("to_id") . " -");
        }
    ...
    protected function get_status_dropdown($show_header = false) {
    
        $client_groups = $this->Status_master_model->get_all()->result();
        $groups_dropdown = array();

        if ($show_header) {
            $groups_dropdown[] = array("id" => "", "text" => "- " . lang("status") . " -");
        }
    ...
    protected function get_complete_dropdown($show_header = false) {
    
        $client_groups = $this->Complete_master_model->get_all()->result();
        $groups_dropdown = array();
     ...
     protected function get_giftfor_dropdown($show_header = false) {
    
        $client_groups = $this->Giftfor_master_model->get_all()->result();
        $groups_dropdown = array();

        if ($show_header) {
            $groups_dropdown[] = array("id" => "", "text" => "- " . lang("gift_for") . " -");
        }
...
protected function get_telemarketers_dropdown($show_header = false) {
    
        $client_groups = $this->Telemarketers_model->get_all()->result();
        $groups_dropdown = array();

        if ($show_header) {
            $groups_dropdown[] = array("id" => "", "text" => "- " . lang("telemarketers") . " -");
        }
