...
class Dashboard extends MY_Controller {
    ...
    public function index() {
        ...
        if ($this->login_user->user_type === "staff" && $this->show_staff_on_staff) {
            ...
            $view_data['totalclient'] = $this->Clients_model->get_total_client();
            $view_data['totalcus'] = $this->Clients_model->get_total();
            $view_data['totalsales'] = $this->Clients_model->get_total_salesmanager();
            $view_data['totaltele'] = $this->Clients_model->get_total_telecallers();

            //TelemarketingManager
            $view_data['totalassigncus'] = $this->Customers_model->get_total_assigncus();
            $view_data['totalunassigncus'] = $this->Customers_model->get_total_unassigncus();
            $view_data['totalcompletecus'] = $this->Customers_model->get_total_completecus();
            $view_data['totalincompletecus'] = $this->Customers_model->get_total_incompletecus();
            //GiftManager
            $view_data['totalgiftprocessmonth'] = $this->Customers_model->get_total_totalgiftprocessmonth();
            $view_data['totalbdaygiftmonth'] = $this->Customers_model->get_total_totalbdaygiftmonth();
            $view_data['totalanngiftmonth'] = $this->Customers_model->get_total_totalanngiftmonth();
            //CourierManager
            $view_data['totalgiftsent'] = $this->Customers_model->get_total_totalgiftsent();
            $view_data['totalgiftunsent'] = $this->Customers_model->get_total_totalgiftunsent();
            $view_data['totalgiftdeliver'] = $this->Customers_model->get_total_totalgiftdeliver();
            $view_data['totalgiftundeliver'] = $this->Customers_model->get_total_totalgiftundeliver();
            //Accounts

            // $view_data['totalinvoice'] = $this->Customers_model->get_total_totalinvoice();
            $options = 0;        
            $list_data = $this->Customers_model->get_total_totalinvoice($options)->result();
            $invoice = 0;
            foreach ($list_data as $data) {
                $invoice = $invoice  + $data->invoice_value;
            }
            $view_data['totalinvoice'] = $invoice;




            //$view_data['totalincome'] = $this->Customers_model->get_total_totalincome();
            $options = 0;
            $list_data = $this->Customers_model->get_total_totalinvoice($options)->result();
            $payments = 0;
            foreach ($list_data as $data) {
                $payments = $payments  + $data->payment_received;
            }
            $view_data['totalincome'] = $payments;



            //$view_data['totalexp'] = $this->Customers_model->get_total_totalexp();
            $options = 0;
            $list_data = $this->Customers_model->get_total_totalexp($options)->result();
            $expenses = 0;
            $expensess = 0;
            $tax = 0;
            $tax2 = 0;
            foreach ($list_data as $data) {
                ...
                $expenses = $data->amount + $tax + $tax2;
                $expensess = $expensess + $expenses;
            }
            $view_data['totalexp'] = $expensess;
            ...
            //$view_data['totalcash'] = $this->Customers_model->get_total_totalcash();


            $options = 1;
            //$view_data['totalinvoicemonth'] = $this->Customers_model->get_total_totalinvoicemonth();
            $list_data = $this->Customers_model->get_total_totalinvoice($options)->result();
            $invoicem = 0;
            foreach ($list_data as $data) {
                $invoicem = $invoicem  + $data->invoice_value;
            }
            $view_data['totalinvoicemonth'] = $invoicem;




            $options = 1;
            //$view_data['totalincomemonth'] = $this->Customers_model->get_total_totalincomemonth();
            $list_data = $this->Customers_model->get_total_totalinvoice($options)->result();
            $paymentsm = 0;
            foreach ($list_data as $data) {
                $paymentsm = $paymentsm  + $data->payment_received;
            }
            $view_data['totalincomemonth'] = $paymentsm;
            ...
            //$view_data['totalexpmonth'] = $this->Customers_model->get_total_totalexpmonth();
            $options = 1;
            $list_data = $this->Customers_model->get_total_totalexp($options)->result();
            $expensesm = 0;
            $expensessm = 0;
            ...
            foreach ($list_data as $data) {
                ...
                $expensesm = $data->amount + $tax + $tax2;
                $expensessm = $expensessm + $expensesm;
            }
            $view_data['totalexpmonth'] = $expensessm;
            ...
            } else if($this->login_user->role_id == "4") { //TelemarketingManager
            ...
            } else if($this->login_user->role_id == "3") { //Telecallers
        ...
        //            }
            ...
            $view_data['totalassigncus'] = $this->Customers_model->get_total_assigncus($telemarketer_id);
            $view_data['totalcompletecus'] = $this->Customers_model->get_total_completecus($telemarketer_id);
            $view_data['totalincompletecus'] = $this->Customers_model->get_total_incompletecus($telemarketer_id);
            $view_data['assigntoday'] = $this->Customers_model->get_total_assigntoday($telemarketer_id);
            $view_data['completetoday'] = $this->Customers_model->get_total_completetoday($telemarketer_id);

            } else if($this->login_user->role_id == "2") { //Salesmanager

            $salesmanager_id = $this->login_user->salesmanager_id;

            $view_data['totalclients'] = $this->Customers_model->get_total_clientsales($salesmanager_id);
            $view_data['totalsalescus'] = $this->Customers_model->get_total_totalsalescus($salesmanager_id);
            $view_data['totalgiftsmonths'] = $this->Customers_model->get_total_totalgiftsmonths($salesmanager_id);
            $this->Customers_model->get_salesmanqty($salesmanager_id);
            $view_data['totalcommmonths'] = $this->Customers_model->searchcom($salesmanager_id);
            // $view_data['totalincompletecus'] = $this->Customers_model->get_total_incompletecus($salesmanager_id);
            // $view_data['assigntoday'] = $this->Customers_model->get_total_assigntoday($salesmanager_id);
            // $view_data['completetoday'] = $this->Customers_model->get_total_completetoday($salesmanager_id);

            } else if($this->login_user->role_id == "8") { //GiftManager
            ...
            } else if($this->login_user->role_id == "6") {  //CourierManager
            ...
            } else if($this->login_user->role_id == "5") {  //Accounts

            
            //$view_data['totalinvoicetoday'] = $this->Customers_model->get_total_totalinvoicetoday();
            $options = 2;
            $list_data = $this->Customers_model->get_total_totalinvoice($options)->result();
            $invoiced = 0;
            foreach ($list_data as $data) {
                $invoiced = $invoiced  + $data->invoice_value;
            }
            $view_data['totalinvoicetoday'] = $invoiced;


            //$view_data['totalincometoday'] = $this->Customers_model->get_total_totalincometoday();
            $options = 2;
            $list_data = $this->Customers_model->get_total_totalinvoice($options)->result();
            $paymentsd = 0;
            foreach ($list_data as $data) {
                $paymentsd = $paymentsd  + $data->payment_received;
            }
            $view_data['totalincometoday'] = $paymentsd;




            //$view_data['totalexptoday'] = $this->Customers_model->get_total_totalexptoday();
            $options = 2;
            $list_data = $this->Customers_model->get_total_totalexp($options)->result();
            $expensesd = 0;
            $expensessd = 0;
            ...
            foreach ($list_data as $data) {
                ...
                $expensesd = $data->amount + $tax + $tax2;
                $expensessd = $expensessd + $expensesd;
            }
            $view_data['totalexptoday'] = $expensessd;
    ...
    private function _check_widgets_for_clients() {
        ...
        if ($show_invoice_info) {
            ...
            if (!in_array("package", $hidden_menu)) {
                $widget["package"] = true;
            }
            if (!in_array("package_types", $hidden_menu)) {
                $widget["package_types"] = true;
            }
            if (!in_array("total_gifts", $hidden_menu)) {
                $widget["total_gifts"] = true;
            }
    ...
    private function _get_widgets_for_staffs($widget, $widgets_array) {
        if (get_array_value($widgets_array, $widget)) {
            if ($widget == "clock_in_out") {
                ...
                return get_invoices_value_widget($value, true);
                ...
                return pending_leave_approval_widget(true, array("access_type" => $this->access_type, "allowed_members" => $this->allowed_members));
            } else if ($widget == "total_hours_worked" || $widget == "total_project_hours") {
                return count_total_time_widget_small(0, $widget, true);
            } else if ($widget == "draft_invoices") {
                return draft_invoices_widget(true);
            }
        } else {
    ...
    private function _get_widgets_for_client($widget, $widgets_array) {
        ...
        if (get_array_value($widgets_array, $widget)) {
            if ($widget == "total_projects") {
                ...
                return $this->load->view("clients/info_widgets/tab", array("tab" => "package", "client_info" => $client_info), true);
            } else if ($widget == "package_types") {
                return $this->load->view("clients/info_widgets/tab", array("tab" => "package_types", "client_info" => $client_info), true);
            } else if ($widget == "total_gifts") {
                return $this->load->view("clients/info_widgets/tab", array("tab" => "total_gifts", "client_info" => $client_info), true);
    ...
    function client_default_dashboard() {
        ...
        if ($client_default_dashboard) {
            ...
            $dashboard_view = $this->load->view("dashboards/custom_dashboards/view", $view_data, true);
        } else {
            $view_data['show_invoice_info'] = get_array_value($widgets, "show_invoice_info");
            $view_data['hidden_menu'] = get_array_value($widgets, "hidden_menu");
            $view_data['client_info'] = get_array_value($widgets, "client_info");
            ...
            $dashboard_view = $this->load->view("dashboards/client_dashboard", $view_data, true);
        }

        $view_data["dashboard_view"] = $dashboard_view;

        $this->template->rander("settings/client_default_dashboard/index", $view_data);
    }
    ...
    function edit_client_default_dashboard() {
        ...
        $this->template->rander("settings/client_default_dashboard/edit_dashboard", $view_data);
    }
    ...
    function restore_to_default_client_dashboard() {
        ...
        redirect("dashboard/client_default_dashboard");
    }
