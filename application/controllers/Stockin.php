...
class Stockin extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("stock/stockin/index", $view_data);
    }
    ...
    function yearly() {
        $this->load->view("stock/stockin/yearly_expenses");
    }

    //load custom expenses list
    function custom() {
        $this->load->view("stock/stockin/custom_expenses");
    }
    ...
    function modal_form() {
       ...
       // $invoice_id = $this->input->post('invoice_id');
        ...
        $this->load->view('stock/stockin/modal_form', $view_data);
    }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($this->login_user->is_admin ) {
                $row_data[] = modal_anchor(get_uri("stockin/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_stock'), "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_stock'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("stockin/delete"), "data-action" => "delete-confirmation"));
            }
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            $this->load->view("stock/stockin/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function yearly_chart() {
        $this->load->view("stock/stockin/yearly_chart");
    }
