...
class Mtelemarketers_model extends Crud_model {
    ...
    function get_details($options = array()) {
       ...
       // // echo $this->db->last_query();
        ...
        $sql11="UPDATE customers set next_followupdate = NULL WHERE next_followupdate ='0000-00-00'";
        $this->db->query($sql11);
        ...
        if ($start_date && $end_date) {
            $where .= " AND ($clients_table.next_followupdate BETWEEN '$start_date' AND '$end_date') ";
        }
        ...
        if ($status_id) {
            ...
            $where .= " AND $clients_table.transfer_id='2'";
        }
        ...
        $sql = "SELECT $clients_table.*, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS primary_contact, $users_table.id AS primary_contact_id, $users_table.image AS contact_avatar,  project_table.total_projects, $payment_value_select AS payment_received $select_custom_fieds,
                ...
                $clientname_table.company_name as client_name,complete_master.title as status
        FROM $clients_table 
    ...
    function get_details_rep($options = array()) {
        ...
        if ($telemarketer_id) {
            ...
            //$where .= " AND FIND_IN_SET('0', $clients_table.id)";
            $where .= " AND $clients_table.telemarketer_id='0'";
        }
        ////$telemarketer_id = $this->login_user->telecallers_id;
        // if ($group_id and $telemarketer_id) {
        //     $where .= " AND FIND_IN_SET('$group_id', $clientname_table.id)";
        ...
        $join_custom_fieds               
        WHERE $clients_table.deleted=0 $where 
         ORDER BY $clients_table.assign_date ASC";
        return $this->db->query($sql);
       // echo $this->db->last_query();
    ...
    function insert_datas($telemarketer_id, $client_id, $from_id, $to_id) {
        ...
        // $sql = "INSERT INTO giftdatas (telemarketer_id,customer_id,teledatas_id,client_id,customer_name,birth_date,            anniversary_date,address,city,landmark,state,zip,phone,email)
        //         SELECT telemarketer_id,customer_id,id,client_id,customer_name,birth_date,anniversary_date,address,city,landmark,state,zip,phone,email FROM teledatas
        //         WHERE teledatas.id >= $from_id AND teledatas.id <= $to_id and teledatas.client_id = $client_id and teledatas.telemarketer_id = $telemarketer_id and teledatas.complete_id = 1";

        //  $this->db->query($sql);

         // $sqll = "UPDATE  teledatas set status_id = 2 
         // WHERE teledatas.id >= $from_id AND teledatas.id <= $to_id and teledatas.client_id = $client_id and 
         // teledatas.telemarketer_id = $telemarketer_id and teledatas.complete_id = 1";

         $sqll = "UPDATE  customers set transfer_id = 1 , completed_date = now()
         WHERE customers.id >= $from_id AND customers.id <= $to_id and customers.client_id = $client_id and 
         customers.telemarketer_id = $telemarketer_id and customers.complete_id = 1 and customers.transfer_id = 2";
         $this->db->query($sqll);


         $sqllc = "UPDATE customers
                    INNER JOIN clients ON customers.client_id = clients.id
                    SET customers.salesmanager_id = clients.salesmanager_id
         WHERE customers.id >= $from_id AND customers.id <= $to_id and customers.client_id = $client_id and 
         customers.telemarketer_id = $telemarketer_id and customers.complete_id = 1 and customers.transfer_id = 2";
         $this->db->query($sqllc);
